...
async function loadSignupLink(token: string) {
  const admin = createAdminClient();
  const { data, error } = await admin
    .from("driver_signup_links")
    .select(
      "id,company_id,public_token,link_name,status,expires_at,max_applications,application_count,companies(id,name,company_type,status,address,phone,email,logo_path,primary_colour,secondary_colour,accent_colour)",
    )
...
async function startApplication(
  ...
  if (email && !email.includes("@")) return fail("Enter a valid email address or leave it blank.");
  ...
  if (!payNowType || !["mobile", "nric", "uen", "other"].includes(payNowType) || !payNowNo) {
  ...
  const { data: application, error: insertError } = await admin
    ...
    .insert({
      ...
      nric_passport: clean(body.nric_passport),
      nationality: clean(body.nationality),
      date_of_birth: cleanDate(body.date_of_birth),
      licence_no: clean(body.licence_no),
      licence_class: clean(body.licence_class),
