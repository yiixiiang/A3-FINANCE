...
export default function DriverNetworkPage() {
  ...
  function signupUrl(token: string) {
    return `${origin || ""}/driver-signup/${token}`;
  }
  ...
  async function copyText(value: string) {
    try {
      ...
      window.prompt("Copy this signup link:", value);
    }
  }
  ...
  async function createSignupLink() {
    ...
    const token = result?.signup_link?.public_token as string | undefined;
    if (token) {
      const url = signupUrl(token);
      setLatestLink(url);
      await copyText(url);
    }
              ...
              <>
                <article className={styles.profileCard}>
                  ...
                  <div className={styles.profileFacts}>
                    <div><span>Licence</span><strong>{selectedDriver.licence_no || "—"}</strong><small>{selectedDriver.licence_class || "Class not set"}</small></div>
      ...
      {tab === "signup" ? (
        <section className={styles.signupLayout}>
          <article className={styles.signupBuilder}>
            ...
            <div className={styles.lockNotice}><strong>Company lock enabled</strong><p>The URL token controls the company. No company selector is included on the public signup form.</p></div>
            ...
            {latestLink ? <div className={styles.latestLink}><span>Latest link</span><code>{latestLink}</code><button type="button" onClick={() => void copyText(latestLink)}>Copy</button></div> : null}
          </article>
          ...
          <article className={styles.linkListCard}>
            ...
            <div className={styles.linkList}>
              {data.signup_links.map((link) => {
                ...
                const url = signupUrl(link.public_token);
                ...
                return (
                  <article key={link.id}>
                    ...
                    <div className={styles.linkActions}>
                      <button type="button" onClick={() => void copyText(url)}>Copy</button>
      ...
      {tab === "applications" ? (
        <section className={styles.applicationsCard}>
          <div className={styles.applicationsHeader}>
            <div><span>Recruitment inbox</span><h2>Driver Applications</h2><p>Approve an applicant to create or link the driver under the company selected by the signup URL.</p></div>
              ...
              return (
                <article key={application.id}>
                  ...
                  <div className={styles.applicationDetails}>
                    ...
                    <div><span>Licence</span><strong>{application.licence_no || "—"}</strong><small>{application.licence_class || "Class not provided"} · Expiry {formatDate(application.licence_expiry)}</small></div>
                  ...
                  {application.status === "pending" ? (
                    <div className={styles.applicationActions}>
                      ...
                      <button type="button" className={styles.approveButton} disabled={saving} onClick={() => void reviewApplication(application, true)}>Approve & Add Driver</button>
                    </div>
                  ) : null}
                </article>
              );
