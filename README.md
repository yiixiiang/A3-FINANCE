...
- Short company-locked driver recruitment links at `/d/CODE`
- Company-scoped administrator and User Management access

## Version 1.3 changes

- The public `/limousine` website is removed from A3 Finance. The standalone limousine site may continue using the retained public quotation API.
- New driver recruitment links use the short route `/d/CODE`; the admin screen displays the short path and copies the complete working link.
- Approving a driver application creates the driver authentication account automatically when no login exists. The temporary password is shown once to the administrator.
- Public driver recruitment no longer collects NRIC/passport or driving licence number, class or expiry.
- Administrators and User Management are restricted to explicitly assigned companies. Cross-company user records are not returned.
...
Run the migrations in filename order from `000_phase0_foundation.sql` through `024_company_scoped_admin_short_driver_links.sql`, with one exception:
...
Then run migrations `020`, `021`, `022`, `023` and `024` in order.
