...
const requiredMigrations = [
  ...
  "024_company_scoped_admin_short_driver_links.sql",
];
...
if (!failures.some((item) => item.includes("migration"))) {
  pass("Migrations 017 through 024 are present and transaction-wrapped.");
}
...
if (
  ...
  !driverLoginRepairSql.includes("select distinct") ||
  !driverLoginRepairSql.includes("update public.user_company_access access") ||
  !driverLoginRepairSql.includes("where not exists")
) {
  fail("Migration 023 does not safely repair and deduplicate exact-email driver login links.");
} else {
  pass("Driver login repair uses exact-email, role and cardinality-safe update/insert safeguards.");
}
...
const requiredPages = [
  ...
  "src/components/driver-signup-form.tsx",
...
if (!requiredPages.some((file) => !fs.existsSync(path.join(root, file)))) {
  pass("Phase 8–13 pages, scoped administration, short signup routes and public APIs are present.");
}
...
const publicSignupPage = fs.readFileSync(path.join(root, "src/components/driver-signup-form.tsx"), "utf8");
...
if (fs.existsSync(path.join(root, "src/app/limousine"))) {
  fail("The public limousine website must not be bundled inside A3 Finance.");
}
if (enterpriseShell.includes('href: "/limousine"') || enterpriseShell.includes('pathname === "/limousine"')) {
  fail("EnterpriseShell still exposes the removed public limousine website.");
}
if (!publicLimousineApi.includes('from("limousine_quote_requests")')) {
  fail("The standalone limousine website API integration is missing.");
}
if (!publicLimousineSql.includes("public.limousine_quote_requests")) {
  fail("Migration 022 does not retain standalone limousine quotation storage.");
}
if (!failures.some((item) => item.includes("public limousine") || item.includes("EnterpriseShell") || item.includes("limousine website API") || item.includes("Migration 022"))) {
  pass("Public limousine pages are removed from Finance while the standalone site API remains available.");
}

const phase13Sql = fs.readFileSync(path.join(migrationDirectory, "024_company_scoped_admin_short_driver_links.sql"), "utf8");
const driverNetworkApi = fs.readFileSync(path.join(root, "src/app/api/admin/driver-network/route.ts"), "utf8");
const userAdminApi = fs.readFileSync(path.join(root, "src/app/api/admin/users/route.ts"), "utf8");
const driverNetworkPage = fs.readFileSync(path.join(root, "src/app/driver-network/page.tsx"), "utf8");
for (const requiredText of [
  "short_code",
  "current_user_company_permission",
  "current_user_shares_company",
  "set_active_company",
]) {
  if (!phase13Sql.includes(requiredText)) fail(`Migration 024 is missing ${requiredText}.`);
}
if (!driverNetworkApi.includes("createOrAttachDriverLogin") || !driverNetworkApi.includes("temporary_password")) {
  fail("Driver approval does not automatically create or attach a driver login.");
}
if (!userAdminApi.includes("requireAdministratorScope") || !userAdminApi.includes("targetIsFullyInScope")) {
  fail("User Management is not restricted to the administrator's assigned companies.");
}
if (!driverNetworkPage.includes("/d/${code}") || !driverNetworkPage.includes("short_code")) {
  fail("Driver Network does not use the short /d/CODE signup route.");
}
if (/nric_passport|licence_no|licence_class|licence_expiry/.test(publicSignupPage)) {
  fail("Public driver signup still collects NRIC or driving licence fields.");
}
if (!failures.some((item) => item.includes("Migration 024") || item.includes("Driver approval") || item.includes("User Management") || item.includes("short /d/CODE") || item.includes("still collects"))) {
  pass("Company-scoped administrators, short signup codes, automatic driver accounts and reduced personal data are installed.");
}
...
if (!fs.existsSync(shellPath)) fail("EnterpriseShell is missing.");
  ...
  if (!["/balance-sheet", "/cash-flow", "/financial-control", "/driver-network"].some((href) => !shell.includes(href))) {
    pass("EnterpriseShell includes the required finance and Driver Network navigation.");
  }
...
if (packageJson.version !== "1.3.2") fail("package.json version must be 1.3.2.");
...
console.log("\nA3 Finance version 1.3.2 verification passed.");
