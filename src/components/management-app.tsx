...
export function ManagementApp(){
 ...
 const searchResults=useMemo(()=>{
  ...
  bookings.filter(row=>row.join(" \ ").toLowerCase().includes(term)).slice(0,6).forEach(row=>results.push({id:`booking-${row[0]}`,moduleId:"limousine",title:row[0],detail:`${row[1]} Â· ${row[2]}`}));
 ...
 if(!authReady)return <div className="authloading" role="status"><span className="skeleton skeleton-title"/><span className="sr-only">Loading sign inâ€¦</span></div>;
...
function CloudCenter(){
 ...
 const size=diagnostics?diagnostics.localBytes<1024?`${diagnostics.localBytes} B`:diagnostics.localBytes<1048576?`${(diagnostics.localBytes/1024).toFixed(1)} KB`:`${(diagnostics.localBytes/1048576).toFixed(2)} MB`:"â€”";
 const lastSync=diagnostics?.lastSyncAt||snapshot.lastSyncAt;
 return <><Heading eyebrow="SUPABASE Â· MULTI-DEVICE SAFETY" title="Cloud & Backup" copy="Synchronize safely between computers, review conflicts, and keep automatic cloud backups before data is replaced."/>
 <div className="cloudsummary cloudsummary-v22">
  ...
  <div className="cloudmetric"><span>Local record groups</span><strong>{diagnostics?.localKeyCount??"â€”"}</strong><small>{size}</small></div>
  <div className="cloudmetric"><span>Cloud record groups</span><strong>{diagnostics?.cloudKeyCount??"â€”"}</strong><small>{diagnostics?.email||snapshot.email||"Not signed in"}</small></div>
...
function ModuleLoading(){return <div className="panel moduleloading" role="status" aria-live="polite"><span className="skeleton skeleton-title"/><span className="skeleton skeleton-copy"/><span className="skeleton skeleton-grid"/><span className="sr-only">Loading workspaceâ€¦</span></div>}
