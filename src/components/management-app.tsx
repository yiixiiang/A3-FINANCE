...
import { LayoutDashboard, CarFront, UtensilsCrossed, WalletCards, Percent, Network, Settings2, BarChart3, ReceiptText, FileText, Search, Bell, Menu, X, ChevronRight, Plus, TrendingUp, Users, Clock3, ShieldCheck, Globe2, Banknote, LogOut, Database, CloudUpload, CloudDownload, RefreshCw, HardDriveDownload, CheckCircle2, AlertTriangle, History, Download, Trash2, CalendarDays, Eye, Save, LoaderCircle } from "lucide-react";
import { bookings as legacyBookings } from "@/lib/data";
...
const nav: {label:string;items:Item[]}[]=[
 ...
 {label:"Booking",items:[{id:"bookings",label:"Booking Management",icon:CalendarDays}]},
...
const corePages=new Set(["overview","bookings","cloud"]);
...
export function ManagementApp(){
 ...
 const searchResults=useMemo(()=>{
  ...
  legacyBookings.filter(row=>row.join(" ").toLowerCase().includes(term)).slice(0,6).forEach(row=>results.push({id:`booking-${row[0]}`,moduleId:"bookings",title:row[0],detail:`${row[1]} · ${row[2]}`}));
 ...
 return <div className={mobile?"shell menu-open":"shell"} aria-busy={isNavigating}>
  ...
  <main>
   ...
   <section className="content">
    ...
    {canViewActive&&active==="overview"&&<Overview role={roleLabel(currentUser.role)}/>} {canViewActive&&active==="bookings"&&<BookingManagement query={deferredQuery}/>} {canViewActive&&active==="cloud"&&<CloudCenter/>}
...
function Overview({role}:{role:string}){
 ...
 const snapshot=useMemo(()=>{
  ...
  const activeBookings=legacyBookings.filter(row=>!["Completed","Cancelled"].includes(row[5])).length;
  const pendingBookings=legacyBookings.filter(row=>row[5]==="Pending").length;
...
type LiveBooking={
 id?:string;reference?:string;kind:"limousine"|"sakura";createdAt?:string;updatedAt?:string;status?:string;
 name?:string;customer?:string;contact?:string;email?:string;date?:string;time?:string;pickup?:string;destination?:string;
 service?:string;vehicle?:string;amount?:number|string;driver?:string;notes?:string;guests?:number|string;occasion?:string;table?:string;
 depositStatus?:string;paymentStatus?:string;
};
const BOOKING_STATUSES=["Pending","Confirmed","Driver assigned","Completed","Cancelled"];
const bookingReference=(booking:LiveBooking)=>String(booking.reference||booking.id||"—");
const bookingCustomer=(booking:LiveBooking)=>String(booking.name||booking.customer||"—");
const bookingDateTime=(booking:LiveBooking)=>[booking.date,booking.time].filter(Boolean).join(" · ")||new Date(booking.createdAt||Date.now()).toLocaleString("en-SG");
const bookingAmount=(booking:LiveBooking)=>{const value=Number(String(booking.amount??"").replace(/[^0-9.-]/g,""));return Number.isFinite(value)&&value>0?money(value):"—"};
function BookingManagement({query}:{query:string}){
 const [records,setRecords]=useState<LiveBooking[]>([]);const [filter,setFilter]=useState("all");const [loading,setLoading]=useState(true);const [error,setError]=useState("");const [editing,setEditing]=useState<LiveBooking|null>(null);const [saving,setSaving]=useState(false);
 const loadBookings=async()=>{setLoading(true);setError("");try{const response=await fetch("/api/admin/bookings",{cache:"no-store"});const payload=await response.json();if(!response.ok||!payload.ok)throw new Error(payload.message||"Unable to load bookings.");setRecords(Array.isArray(payload.bookings)?payload.bookings:[])}catch(err){setError(err instanceof Error?err.message:"Unable to load bookings.")}finally{setLoading(false)}};
 useEffect(()=>{void loadBookings()},[]);
 const filtered=useMemo(()=>records.filter(record=>{const term=query.trim().toLowerCase();const matchesSearch=!term||JSON.stringify(record).toLowerCase().includes(term);if(!matchesSearch)return false;if(filter==="all")return true;if(filter==="limousine"||filter==="sakura")return record.kind===filter;return String(record.status||"Pending").toLowerCase()===filter;}),[records,filter,query]);
 const counts=useMemo(()=>({all:records.length,limousine:records.filter(x=>x.kind==="limousine").length,sakura:records.filter(x=>x.kind==="sakura").length,pending:records.filter(x=>(x.status||"Pending").toLowerCase()==="pending").length,confirmed:records.filter(x=>(x.status||"").toLowerCase()==="confirmed").length,completed:records.filter(x=>(x.status||"").toLowerCase()==="completed").length,cancelled:records.filter(x=>(x.status||"").toLowerCase()==="cancelled").length}),[records]);
 const saveBooking=async()=>{if(!editing)return;setSaving(true);setError("");try{const response=await fetch("/api/admin/bookings",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(editing)});const payload=await response.json();if(!response.ok||!payload.ok)throw new Error(payload.message||"Unable to save booking.");setRecords(items=>items.map(item=>bookingReference(item)===bookingReference(editing)&&item.kind===editing.kind?payload.booking:item));setEditing(null)}catch(err){setError(err instanceof Error?err.message:"Unable to save booking.")}finally{setSaving(false)}};
 const removeBooking=async(record:LiveBooking)=>{if(!window.confirm(`Delete ${bookingReference(record)} permanently?`))return;setError("");const response=await fetch("/api/admin/bookings",{method:"DELETE",headers:{"Content-Type":"application/json"},body:JSON.stringify({reference:bookingReference(record),kind:record.kind})});const payload=await response.json();if(!response.ok||!payload.ok){setError(payload.message||"Unable to delete booking.");return}setRecords(items=>items.filter(item=>!(bookingReference(item)===bookingReference(record)&&item.kind===record.kind)))};
 const tabs=[["all","All bookings"],["limousine","Limousine"],["sakura","Sakura table"],["pending","Pending"],["confirmed","Confirmed"],["completed","Completed"],["cancelled","Cancelled"]] as const;
 return <><Heading eyebrow="A3 GROUP · CENTRAL OPERATIONS" title="Booking Management" copy="Live Limousine and Sakura website bookings in one operational workspace."/><div className="booking-summary"><div><span>Total</span><strong>{counts.all}</strong></div><div><span>Pending</span><strong>{counts.pending}</strong></div><div><span>Confirmed</span><strong>{counts.confirmed}</strong></div><div><span>Completed</span><strong>{counts.completed}</strong></div></div><div className="booking-toolbar"><div className="booking-tabs">{tabs.map(([id,label])=><button key={id} className={filter===id?"active":""} onClick={()=>setFilter(id)}>{label}<b>{counts[id]}</b></button>)}</div><button className="ghost" onClick={()=>void loadBookings()} disabled={loading}><RefreshCw size={15}/>{loading?"Refreshing…":"Refresh"}</button></div>{error&&<div className="cloudnotice error">{error}</div>}<div className="panel booking-panel">{loading?<div className="empty compact"><LoaderCircle className="spin" size={32}/><p>Loading live bookings…</p></div>:filtered.length?<div className="tablewrap"><table className="booking-table"><thead><tr>{["Reference","Source","Customer","Schedule","Booking details","Status","Amount","Actions"].map(item=><th key={item}>{item}</th>)}</tr></thead><tbody>{filtered.map(record=><tr key={`${record.kind}-${bookingReference(record)}`}><td><strong>{bookingReference(record)}</strong><small>{record.createdAt?new Date(record.createdAt).toLocaleDateString("en-SG"):""}</small></td><td><span className={`sourcepill ${record.kind}`}>{record.kind==="limousine"?"Limousine":"Sakura"}</span></td><td>{bookingCustomer(record)}<small>{record.contact||record.email||"—"}</small></td><td>{bookingDateTime(record)}</td><td>{record.kind==="limousine"?<>{record.service||"—"}<small>{record.vehicle||"—"} · {record.pickup||"—"} → {record.destination||"—"}</small></>:<>{record.guests||"—"} guests<small>{record.occasion||record.table||"Table not assigned"}</small></>}</td><td><span className={`status ${String(record.status||"Pending").toLowerCase().replace(/\s+/g,"")}`}>{record.status||"Pending"}</span></td><td>{bookingAmount(record)}</td><td><div className="rowactions"><button className="rowaction" onClick={()=>setEditing({...record})}><Eye size={14}/>Open</button><button className="rowaction danger" onClick={()=>void removeBooking(record)}><Trash2 size={14}/>Delete</button></div></td></tr>)}</tbody></table></div>:<div className="empty"><CalendarDays size={38}/><h2>No bookings found</h2><p>New website submissions will appear here automatically. No demonstration records are shown.</p></div>}</div>{editing&&<div className="modalbackdrop" onMouseDown={event=>{if(event.target===event.currentTarget)setEditing(null)}}><div className="modal booking-modal"><div className="modalhead"><div><span>{editing.kind==="limousine"?"LIMOUSINE BOOKING":"SAKURA TABLE BOOKING"}</span><h2>{bookingReference(editing)}</h2></div><button onClick={()=>setEditing(null)} aria-label="Close"><X/></button></div><div className="booking-detail-grid"><section><span>Customer</span><strong>{bookingCustomer(editing)}</strong><small>{editing.contact||editing.email||"No contact provided"}</small></section><section><span>Schedule</span><strong>{bookingDateTime(editing)}</strong><small>{editing.createdAt?`Submitted ${new Date(editing.createdAt).toLocaleString("en-SG")}`:""}</small></section></div><div className="formgrid"><label>Status<select value={editing.status||"Pending"} onChange={e=>setEditing({...editing,status:e.target.value})}>{BOOKING_STATUSES.map(status=><option key={status}>{status}</option>)}</select></label>{editing.kind==="limousine"?<><label>Assigned driver<input value={editing.driver||""} onChange={e=>setEditing({...editing,driver:e.target.value})} placeholder="Driver name or ID"/></label><label>Confirmed amount<input type="number" min="0" step="0.01" value={editing.amount??""} onChange={e=>setEditing({...editing,amount:e.target.value})} placeholder="0.00"/></label><label>Payment status<select value={editing.paymentStatus||"Unpaid"} onChange={e=>setEditing({...editing,paymentStatus:e.target.value})}><option>Unpaid</option><option>Deposit paid</option><option>Paid</option><option>Refunded</option></select></label></>:<><label>Table assignment<input value={editing.table||""} onChange={e=>setEditing({...editing,table:e.target.value})} placeholder="Example: T12"/></label><label>Deposit status<select value={editing.depositStatus||"Unpaid"} onChange={e=>setEditing({...editing,depositStatus:e.target.value})}><option>Unpaid</option><option>Paid</option><option>Refunded</option></select></label></>}<label className="wide">Internal notes<textarea rows={4} value={editing.notes||""} onChange={e=>setEditing({...editing,notes:e.target.value})} placeholder="Operational notes, guest requests, driver instructions…"/></label></div><div className="modalactions"><button className="ghost" onClick={()=>setEditing(null)}>Cancel</button><button className="primary" onClick={()=>void saveBooking()} disabled={saving}><Save size={16}/>{saving?"Saving…":"Save booking"}</button></div></div></div>}</>;
}
