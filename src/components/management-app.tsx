...
function BookingManagement({query}:{query:string}){
 ...
 return <><Heading eyebrow="A3 GROUP · LIVE DISPATCH" title="Booking Management" copy="Review every booking, route, fare and assignment from one operational workspace."/>
 ...
 <div className="dispatchworkspace">
  ...
  <main className="dispatchdetail">
   {!selected?<div className="dispatchblank"><Eye size={38}/><h2>Select a booking</h2><p>Choose a booking on the left to view the complete operational record.</p></div>:<>
    ...
    <div className="dispatchsummary"><div><small>Customer</small><strong>{String(selected.name||selected.customer||"—")}</strong></div><div><small>Pickup / reservation</small><strong>{String(selected.date||"—")} · {String(selected.time||"—")}</strong></div><div><small>Current fare</small><strong>{money(fare)}</strong></div><div><small>Payment</small><strong>{String(selected.paymentStatus||selected.depositStatus||"Unpaid")}</strong></div></div>
