...
import { LayoutDashboard, CarFront, UtensilsCrossed, WalletCards, Percent, Network, Settings2, BarChart3, ReceiptText, FileText, Search, Bell, Menu, X, ChevronRight, Plus, TrendingUp, Users, Clock3, ShieldCheck, Globe2, Banknote, LogOut, Database, CloudUpload, CloudDownload, RefreshCw, HardDriveDownload, CheckCircle2, AlertTriangle, History, Download, Trash2, CalendarDays, Eye, Save, MapPin, Phone, Mail, UserRoundCheck } from "lucide-react";
...
const nav: {label:string;items:Item[]}[]=[
 ...
 {label:"Fleet & Settings",items:[{id:"ratemanagement",label:"Rate Management",icon:Settings2},{id:"clientsetup",label:"Client Management",icon:Users},{id:"catalogue",label:"Website Catalogue",icon:Globe2},{id:"company",label:"Company Management",icon:Settings2},{id:"cloud",label:"Cloud & Backup",icon:Database},{id:"access",label:"User Access",icon:ShieldCheck}]},
...
function BookingManagement({query}:{query:string}){
 ...
 const tabs=[["all","All Bookings"],["limousine","Limousine"],["sakura","Sakura Table"],["pending","Pending"],["confirmed","Confirmed"],["completed","Completed"],["cancelled","Cancelled"]];
 return <><Heading eyebrow="A3 GROUP · CENTRAL OPERATIONS" title="Booking Management" copy="Open, manage and track live Limousine and Sakura bookings from one workspace."/><div className="bookingtoolbar"><div className="bookingtabs">{tabs.map(([id,label])=><button key={id} className={filter===id?"active":""} onClick={()=>setFilter(id)}>{label}<span>{rows.filter(r=>id==="all"?true:id==="limousine"||id==="sakura"?r.bookingType===id:String(r.status||"Pending").toLowerCase()===id).length}</span></button>)}</div><button className="ghost" onClick={()=>void loadBookings()}><RefreshCw size={15}/>Refresh</button></div>{error&&<div className="panel bookingerror"><AlertTriangle size={18}/>{error}</div>}<div className="panel bookingpanel">{loading?<div className="empty"><RefreshCw className="spin"/><p>Loading live bookings…</p></div>:filtered.length===0?<div className="empty"><CalendarDays size={36}/><h2>No bookings found</h2><p>New website submissions will appear here automatically.</p></div>:<div className="tablewrap"><table className="bookingtable"><thead><tr><th>Reference</th><th>Type</th><th>Customer</th><th>Date & time</th><th>Service / guests</th><th>Status</th><th>Amount</th><th></th></tr></thead><tbody>{filtered.map(row=><tr key={`${row.bookingType}-${row.id}`} className={row.unread!==false?"booking-new":""} onClick={()=>void open(row)}><td><strong>{row.reference||row.id}</strong>{row.unread!==false&&<span className="newbadge">NEW</span>}{urgent(row)&&<span className="urgentbadge">URGENT</span>}</td><td>{row.bookingType==="limousine"?"Limousine":"Sakura"}</td><td>{String(row.name||row.customer||"—")}<small>{String(row.contact||row.phone||row.email||"")}</small></td><td>{String(row.date||"—")} · {String(row.time||"—")}</td><td>{row.bookingType==="limousine"?String(row.service||row.vehicle||"—"):`${String(row.guests||"—")} guests`}</td><td><span className={`status ${String(row.status||"pending").toLowerCase().replace(/\s+/g,"")}`}>{String(row.status||"Pending")}</span></td><td>{money(Number(row.confirmedAmount||row.estimatedTotal||row.amount||0))}</td><td><button className="rowaction" onClick={e=>{e.stopPropagation();void open(row)}}><Eye size={14}/>Open</button></td></tr>)}</tbody></table></div>}</div>{selected&&<div className="modalbackdrop" onMouseDown={()=>setSelected(null)}><div className="modal bookingdetailmodal" onMouseDown={e=>e.stopPropagation()}><div className="modalhead"><div><span>{selected.bookingType==="limousine"?"LIMOUSINE BOOKING":"SAKURA TABLE BOOKING"}</span><h2>{selected.reference||selected.id}</h2></div><button className="modalclose" onClick={()=>setSelected(null)}><X size={19}/></button></div><div className="bookingdetailgrid"><section><h3>Customer</h3><p><Users size={16}/>{String(selected.name||selected.customer||"—")}</p><p><Phone size={16}/>{String(selected.contact||selected.phone||"—")}</p><p><Mail size={16}/>{String(selected.email||"—")}</p></section><section><h3>Schedule</h3><p><CalendarDays size={16}/>{String(selected.date||"—")} · {String(selected.time||"—")}</p><p><Clock3 size={16}/>Submitted {selected.createdAt?new Date(selected.createdAt).toLocaleString("en-SG"):"—"}</p>{urgent(selected)&&<p className="urgenttext"><AlertTriangle size={16}/>Pickup is within 6 hours</p>}</section>{selected.bookingType==="limousine"?<><section className="journeycard"><h3>Journey</h3><div className="routepoint"><span className="routedot pickupdot"></span><div><small>Pickup address</small><strong>{bookingText(selected,"pickup","pickupAddress","pickup_address","pickupLocation","pickup_location","from","origin","originAddress")}</strong></div></div><div className="routeline"></div><div className="routepoint"><span className="routedot dropoffdot"></span><div><small>Destination address</small><strong>{bookingText(selected,"destination","destinationAddress","destination_address","dropoff","dropoffAddress","dropoff_address","dropoffLocation","dropoff_location","to")}</strong></div></div><p><MapPin size={16}/><strong>Additional stops:</strong> {bookingArrayText(selected,"additionalStops","additional_stops","stops","stopAddresses")}</p><p><CarFront size={16}/>{bookingText(selected,"service","serviceType","tripType")} · {bookingText(selected,"vehicle","vehicleName","vehicleType")}</p></section><section><h3>Travel details</h3><p>Passengers: {bookingText(selected,"passengers","pax","passengerCount")}</p><p>Luggage: {bookingText(selected,"luggage","bags","luggageCount")}</p><p>Flight / terminal: {bookingText(selected,"flightNumber","flight_number","flight","terminal")}</p><p>Pickup instructions: {bookingText(selected,"pickupInstructions","pickup_instructions","instructions")}</p><p>Request: {bookingText(selected,"specialRequests","special_requests","remarks","notes")}</p></section></>:<><section><h3>Reservation</h3><p>Guests: {String(selected.guests||"—")}</p><p>Table: {String(selected.tableNumber||"Not assigned")}</p><p>Request: {String(selected.specialRequests||"—")}</p></section></>}</div><div className="bookingeditgrid"><label>Status<select value={String(selected.status||"Pending")} onChange={e=>setSelected({...selected,status:e.target.value})}><option>Pending</option><option>Confirmed</option>{selected.bookingType==="limousine"&&<option>Driver Assigned</option>}<option>Completed</option><option>Cancelled</option></select></label>{selected.bookingType==="limousine"?<label>Assigned driver<input value={String(selected.assignedDriver||"")} onChange={e=>setSelected({...selected,assignedDriver:e.target.value})} placeholder="Driver name or ID"/></label>:<label>Table number<input value={String(selected.tableNumber||"")} onChange={e=>setSelected({...selected,tableNumber:e.target.value})}/></label>}<label>Confirmed amount (S$)<input type="number" min="0" step="0.01" value={Number(selected.confirmedAmount||0)} onChange={e=>setSelected({...selected,confirmedAmount:Number(e.target.value)})}/></label><label>Payment status<select value={String(selected.paymentStatus||selected.depositStatus||"Unpaid")} onChange={e=>setSelected({...selected,paymentStatus:e.target.value})}><option>Unpaid</option><option>Deposit Paid</option><option>Paid</option><option>Refunded</option></select></label><label className="wide">Internal notes<textarea value={String(selected.internalNotes||"")} onChange={e=>setSelected({...selected,internalNotes:e.target.value})}/></label></div><div className="modalactions"><button className="ghost danger" disabled={saving} onClick={()=>void remove()}><Trash2 size={15}/>Delete</button><button className="ghost" onClick={()=>setSelected(null)}>Close</button><button className="primary" disabled={saving} onClick={()=>void save()}><Save size={15}/>{saving?"Saving…":"Save changes"}</button></div></div></div>}</>
}
