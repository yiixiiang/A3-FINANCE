...
type CatalogueRecord={
 id:string;website:string;priceKey:string;item:string;titleZh:string;category:string;subgroup:string;stallId:string;description:string;price:number;currency:string;imageUrl:string;imageAlt:string;displayOrder:number;available:boolean;status:"Published"|"Draft"
};
const catalogueKey=(value:string)=>value.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"");
function normaliseCatalogue(record:Partial<CatalogueRecord>):CatalogueRecord{return {
 id:String(record.id||""),website:String(record.website||"FOOD.A3GROUP.SG"),priceKey:catalogueKey(String(record.priceKey||record.item||"")),item:String(record.item||""),titleZh:String(record.titleZh||""),category:String(record.category||"Food"),subgroup:String(record.subgroup||"Menu"),stallId:String(record.stallId||"chicken-rice"),description:String(record.description||""),price:Number(record.price||0),currency:String(record.currency||"SGD"),imageUrl:String(record.imageUrl||""),imageAlt:String(record.imageAlt||record.item||""),displayOrder:Number(record.displayOrder||0),available:record.available!==false,status:record.status==="Draft"?"Draft":"Published"
}}
function WebsiteCatalogueRate(){
 const [records,setRecords]=useState<CatalogueRecord[]>(()=>load<Partial<CatalogueRecord>[]>("a3-catalogue-rates",[]).map(normaliseCatalogue)); const [editing,setEditing]=useState<CatalogueRecord|null>(null); useEffect(()=>save("a3-catalogue-rates",records),[records]);
 const openNew=()=>setEditing(normaliseCatalogue({website:"FOOD.A3GROUP.SG",category:"Food",subgroup:"Rice Selection",stallId:"chicken-rice",currency:"SGD",status:"Published",available:true}));
 const commit=(record:CatalogueRecord)=>{const normalized=normaliseCatalogue({...record,priceKey:record.priceKey||record.item,imageAlt:record.imageAlt||record.item});if(!normalized.item.trim()||!normalized.priceKey||normalized.price<0)return false;const duplicate=records.some(row=>row.website===normalized.website&&row.priceKey===normalized.priceKey&&row.id!==normalized.id);if(duplicate){window.alert("This Price Key already exists for the selected website.");return false}setRecords(current=>normalized.id?current.map(row=>row.id===normalized.id?normalized:row):[{...normalized,id:nextId("CAT",current.length)},...current]);setEditing(null);return true};
 const remove=(record:CatalogueRecord)=>{if(!window.confirm(`Delete ${record.item} (${record.priceKey})?`))return;setRecords(current=>current.filter(row=>row.id!==record.id))};
 ...
 return <EditableSection eyebrow="ADMIN · WEBSITE CMS" title="Website Food Catalogue" copy="Manage the public menu from Finance: names, descriptions, prices, photos, availability and display order." action="Add menu item" onAdd={openNew} headers={["Record","Website","Photo","Price Key","Item","Group","Rate","Availability","Status","Actions"]} rows={[...records].sort((a,b)=>a.displayOrder-b.displayOrder).map(record=>[record.id,record.website,record.imageUrl?<img src={record.imageUrl} alt={record.imageAlt} style={{width:54,height:54,objectFit:"cover",borderRadius:10}}/>:"—",record.priceKey,record.item,record.subgroup,`${record.currency} ${record.price.toFixed(2)}`,record.available?"Available":"Sold out",record.status,<div className="rowactions" key={record.id}><button className="rowaction" onClick={()=>setEditing({...record})}><Pencil size={14}/>Edit</button><button className="rowaction danger" onClick={()=>remove(record)}>Delete</button></div>])}>{editing&&<RecordModal title={editing.id?"Edit website menu item":"Add website menu item"} onCancel={()=>setEditing(null)} onSave={()=>commit(editing)}>
 <label>Website<select value={editing.website} onChange={e=>setEditing({...editing,website:e.target.value})}><option>FOOD.A3GROUP.SG</option><option>SAKURA.A3GROUP.SG</option><option>LIMOUSINE.A3GROUP.SG</option></select></label>
 <label>Stall<select value={editing.stallId} onChange={e=>setEditing({...editing,stallId:e.target.value})}><option value="chicken-rice">KL Chicken Rice</option><option value="ban-mee">KL Traditional Ban Mee</option><option value="western">27A Western Food</option></select></label>
 <label>Menu group<input value={editing.subgroup} onChange={e=>setEditing({...editing,subgroup:e.target.value})} placeholder="Rice Selection"/></label>
 <label>Price Key<input value={editing.priceKey} onChange={e=>setEditing({...editing,priceKey:catalogueKey(e.target.value)})} placeholder="steamed-chicken-rice"/><small>Must be unique. Existing Food menu keys can be used directly.</small></label>
 <label>English name<input value={editing.item} onChange={e=>setEditing({...editing,item:e.target.value,priceKey:editing.priceKey||catalogueKey(e.target.value),imageAlt:editing.imageAlt||e.target.value})}/></label>
 <label>Chinese name<input value={editing.titleZh} onChange={e=>setEditing({...editing,titleZh:e.target.value})}/></label>
 ...
 <label className="wide">Image URL<input value={editing.imageUrl.startsWith("data:")?"Uploaded image":editing.imageUrl} onChange={e=>setEditing({...editing,imageUrl:e.target.value})} placeholder="https://.../dish.jpg"/><small>Paste a public image URL or upload a small image below.</small></label>
 <label>Upload photo<input type="file" accept="image/jpeg,image/png,image/webp" onChange={e=>uploadImage(e.target.files?.[0]||null)}/></label>
 <label>Image alt text<input value={editing.imageAlt} onChange={e=>setEditing({...editing,imageAlt:e.target.value})}/></label>
 {editing.imageUrl&&<div className="wide"><img src={editing.imageUrl} alt={editing.imageAlt} style={{width:180,height:130,objectFit:"cover",borderRadius:14}}/><button type="button" className="rowaction danger" onClick={()=>setEditing({...editing,imageUrl:""})}>Remove photo</button></div>}
 </RecordModal>}</EditableSection>
}
