...
import { uploadAllLocalDataToCloud, uploadStorageKeysToCloud } from "@/lib/supabase-cloud";
...
function RateManagement(){
 const [tab,setTab]=useState<"limousine"|"driver"|"client">("limousine");
...
function UnifiedLimousineWorkspace(){
 return <div className="limousine-unified">
  <div className="limousine-overview">
   ...
   <div><strong>3. Additional Charges</strong><span>Maintain surcharges displayed below the public rate table.</span></div>
  </div>
  <nav className="limousine-jump" aria-label="Jump to limousine settings"><a href="#limousine-fleet">Fleet & Photos</a><a href="#limousine-rates">Vehicle Rates</a><a href="#limousine-charges">Additional Charges</a></nav>
  <section id="limousine-fleet" className="limousine-section"><FleetManagement/></section>
  <section id="limousine-rates" className="limousine-section"><RateMatrix/></section>
  <section id="limousine-charges" className="limousine-section"><AdditionalCharges/></section>
 </div>
}
...
function synchronizeRateVehiclesWithFleet(fleetInput?:FleetVehicle[]){
 ...
 const oldRules=load<ManagedRateRule[]>(rulesKey,vehicleRateDefaults);
 ...
 const nextRules=oldRules.map(rule=>({...rule,values:nextVehicles.map((name,index)=>{const oldIndex=indexFor(name,index);return oldIndex>=0?String(rule.values?.[oldIndex]??"0"):"0"})}));
 saveNow(vehicleKey,nextVehicles);saveNow(rulesKey,nextRules);
 return {vehicles:nextVehicles,rules:nextRules};
}
...
function ManagedRateMatrix({kind,title,eyebrow,copy,defaults,client}:{kind:string;title:string;eyebrow:string;copy:string;defaults:ManagedRateRule[];client?:string}){
 ...
 useEffect(()=>{if(kind!=="vehicle")return;const synced=synchronizeRateVehiclesWithFleet();setVehicles(synced.vehicles);setRules(synced.rules)},[]);
 ...
 const publishVehicleRates=async(useStarter=false)=>{if(kind!=="vehicle")return;let nextVehicles=vehicles;let nextRules=rules;if(useStarter){nextVehicles=[...rateVehicleDefaults];nextRules=vehicleRateDefaults.map(rule=>({...rule,values:[...rule.values]}));saveNow(vehicleKey,nextVehicles);saveNow(rulesKey,nextRules)}else{const synced=synchronizeRateVehiclesWithFleet();nextVehicles=synced.vehicles;nextRules=synced.rules;setVehicles(nextVehicles);setRules(nextRules)}setPublishState("publishing");try{await uploadStorageKeysToCloud([vehicleKey,rulesKey]);const response=await fetch(`/api/public/rate-matrix?t=${Date.now()}`,{cache:"no-store"});const data=await response.json();if(!response.ok||!Array.isArray(data.rate_cards))throw new Error("Publish verification failed");setPublishState("success")}catch{setPublishState("error")}};
...
function FleetManagement(){
 ...
 const publishFleet=async()=>{const next=normalizedVehicles();setVehicles(next);saveNow(FLEET_STORAGE_KEY,next);synchronizeRateVehiclesWithFleet(next);setPublishState("publishing");setMessage("");try{await uploadStorageKeysToCloud([FLEET_STORAGE_KEY,"a3-rate-management-vehicles-v1","a3-rate-management-vehicle-rules-v1"]);const response=await fetch(`/api/public/rate-matrix?t=${Date.now()}`,{cache:"no-store"});const data=await response.json();if(!response.ok||!Array.isArray(data.vehicle_types))throw new Error(data?.error||"Fleet publish verification failed");const publishedNames=new Set(data.vehicle_types.map((item:any)=>String(item.name||"").trim().toLowerCase()));if(next.filter(item=>item.status==="Active").some(item=>!publishedNames.has(item.name.trim().toLowerCase())))throw new Error("One or more active vehicles were not returned by the public API.");setPublishState("success");setMessage("Fleet, photos, vehicle names and rates published with one synchronized vehicle list.")}catch(error){setPublishState("error");setMessage(error instanceof Error?error.message:"Unable to publish fleet. Check Cloud & Backup connection.")}};
 ...
 return <><Heading eyebrow="ADMIN · PUBLIC FLEET" title="Fleet & Vehicle Photos" copy="Manage vehicle photos, passenger capacity and luggage capacity, then publish them directly to limousine.a3group.sg." action="Add vehicle" onAction={()=>setEditing({id:`VEH-${Date.now()}`,name:"",passengerCapacity:0,luggageCapacity:0,description:"",imageUrl:"",status:"Active",displayOrder:vehicles.length+1})}/><div className="rateactions"><span>Save a draft for later, or publish the latest fleet information to the live limousine website.</span><button className="ghost" disabled={publishState==="publishing"} onClick={saveFleet}><Save size={16}/>{publishState==="saving"?"Saved":"Save draft"}</button><button className="primary" disabled={publishState==="publishing"} onClick={()=>void publishFleet()}><Send size={16}/>{publishState==="publishing"?"Publishing…":"Publish to Limousine Website"}</button></div>{message&&<div className={`publishnotice ${publishState==="error"?"error":""}`}>{publishState==="error"?<X size={17}/>:<CheckCircle2 size={17}/>} {message}</div>}<div className="panel"><div className="tablewrap"><table><thead><tr><th>Photo</th><th>Vehicle</th><th>Passengers</th><th>Luggage</th><th>Status</th><th>Order</th><th>Actions</th></tr></thead><tbody>{[...vehicles].sort((a,b)=>a.displayOrder-b.displayOrder).map(vehicle=><tr key={vehicle.id}><td>{vehicle.imageUrl?<img src={vehicle.imageUrl} alt={vehicle.name} style={{width:92,height:54,objectFit:"cover",borderRadius:8}}/>:"No photo"}</td><td><strong>{vehicle.name}</strong><br/><small>{vehicle.description}</small></td><td>{vehicle.passengerCapacity||"—"}</td><td>{vehicle.luggageCapacity||"—"}</td><td><span className={`status ${vehicle.status.toLowerCase()}`}>{vehicle.status}</span></td><td>{vehicle.displayOrder}</td><td><div className="rowactions"><button className="rowaction" onClick={()=>setEditing({...vehicle})}><Pencil size={14}/>Edit</button><button className="rowaction danger" onClick={()=>removeVehicle(vehicle)}><Trash2 size={14}/>Delete</button></div></td></tr>)}</tbody></table></div></div>{editing&&<RecordModal title={editing.name?"Edit vehicle":"Add vehicle"} onCancel={()=>setEditing(null)} onSave={commit}><label>Vehicle name<input value={editing.name} onChange={e=>setEditing({...editing,name:e.target.value})}/></label><label>Passenger capacity<input type="number" min="0" value={editing.passengerCapacity} onChange={e=>setEditing({...editing,passengerCapacity:Number(e.target.value)})}/></label><label>Luggage capacity<input type="number" min="0" value={editing.luggageCapacity} onChange={e=>setEditing({...editing,luggageCapacity:Number(e.target.value)})}/></label><label>Display order<input type="number" min="1" value={editing.displayOrder} onChange={e=>setEditing({...editing,displayOrder:Number(e.target.value)})}/></label><label>Status<select value={editing.status} onChange={e=>setEditing({...editing,status:e.target.value as FleetVehicle["status"]})}><option>Active</option><option>Inactive</option></select></label><label>Vehicle photo<input type="file" accept="image/*" onChange={e=>void upload(e.target.files?.[0])}/></label><label className="wide">Description<textarea value={editing.description} onChange={e=>setEditing({...editing,description:e.target.value})}/></label>{editing.imageUrl&&<div className="wide"><img src={editing.imageUrl} alt="Vehicle preview" style={{width:"100%",maxHeight:260,objectFit:"cover",borderRadius:12}}/></div>}</RecordModal>}</>
}
...
const siteForCompanyType=(type?:CompanyType):CatalogueSite|undefined=>type==="Food & Beverage"?"food":type==="Nightlife / Entertainment"?"nightclub":undefined;
...
function WebsiteCatalogueRate(){
 ...
 const allowedSites=useMemo<CatalogueSite[]>(()=>{if(access.user.role==="ADMIN")return ["food","nightclub"];const mapped=siteForCompanyType(assignedCompany?.companyType);return mapped?[mapped]:[]},[access.user.role,assignedCompany?.companyType]);
 const [site,setSite]=useState<CatalogueSite>(allowedSites[0]||"food");
 const [records,setRecords]=useState<CatalogueRecord[]>(()=>load<Partial<CatalogueRecord>[]>(WEBSITE_CATALOGUE_STORAGE_KEY,[]).map(normaliseCatalogueV4).filter(record=>record.site!=="limousine"));
 ...
 const [reviews,setReviews]=useState<CatalogueReview[]>(()=>load<CatalogueReview[]>(WEBSITE_REVIEW_STORAGE_KEY,[]).filter(review=>review.site!=="limousine"));
 ...
 return <>
  <Heading eyebrow="ADMIN · WEBSITE CMS" title="Website Catalogue" copy="Manage Food and Nightclub products, prices, pictures, publishing and availability from one place." action={`Add ${siteLabel(site)} item`} onAction={openNew}/>
