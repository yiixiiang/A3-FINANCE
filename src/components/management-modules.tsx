...
type CatalogueStall={id:string;companyId:string;name:string;description:string;imageUrl:string;displayOrder:number;active:boolean;open24Hours:boolean;openTime:string;closeTime:string;closedDays:string[];temporaryClosed:boolean;closureReason:string;phone:string;whatsapp:string;address:string;mapUrl:string};
...
function WebsiteCatalogueRate(){
 ...
 const commit=(record:CatalogueRecord,keepEditing=false)=>{const normalized=normaliseCatalogueV4({...record,companyId:record.companyId||ownerCompanyId,site,priceKey:record.priceKey||record.item,imageAlt:record.imageAlt||record.item});if(!normalized.companyId||!normalized.item.trim()||!normalized.priceKey||normalized.price<0)return false;const duplicate=records.some(row=>row.companyId===normalized.companyId&&row.site===normalized.site&&row.priceKey===normalized.priceKey&&row.id!==normalized.id);if(duplicate){window.alert("This key already exists in this company's catalogue.");return false}let savedRecord=normalized;setRecords(current=>{savedRecord=normalized.id?normalized:{...normalized,id:nextId("CAT",current.length)};return normalized.id?current.map(row=>row.id===normalized.id?savedRecord:row):[savedRecord,...current]});setEditing(keepEditing?{...savedRecord}:null);return true};
 ...
 return <>
  ...
  <div className="documenttoolbar"><strong>{siteLabel(site)} Catalogue</strong><span>{visibleRecords.length} item(s)</span>{site==="food"&&<button className="ghost" onClick={()=>setEditingStall({id:"",companyId:ownerCompanyId,name:"",description:"",imageUrl:"",displayOrder:visibleStalls.length+1,active:true,open24Hours:false,openTime:"",closeTime:"",closedDays:[],temporaryClosed:false,closureReason:"",phone:"",whatsapp:"",address:"",mapUrl:""})}><Plus size={16}/>Add new stall</button>}<button className="rowaction danger" onClick={clearCatalogue}><Trash2 size={15}/>Remove all menu & pictures</button></div>
  ...
  <EditableSection eyebrow={`${siteLabel(site).toUpperCase()} WEBSITE`} title={`${siteLabel(site)} items`} copy={site==="food"?"Menu items and pictures grouped by stall.":site==="nightclub"?"Tower, beer, brandy and any other drink or promotion category.":"Vehicles, transfer services, hourly rates and packages."} action={`Add ${siteLabel(site)} item`} onAdd={openNew} headers={["Photo","Item","Company","Category",site==="food"?"Stall":"Group","Price","Availability","Status","Actions"]} rows={[...visibleRecords].sort((a,b)=>a.displayOrder-b.displayOrder).map(record=>[record.imageUrl?<img src={record.imageUrl} alt={record.imageAlt} style={{width:54,height:54,objectFit:"cover",borderRadius:10}}/>:"—",<div key={record.id}><strong>{record.item}</strong><br/><small>{record.priceKey}</small></div>,companies.find(c=>c.id===record.companyId)?.company||"—",record.category,site==="food"?(visibleStalls.find(s=>s.id===record.stallId)?.name||"Unassigned"):(record.subgroup||"—"),`${record.currency} ${record.price.toFixed(2)}`,record.available?"Available":"Sold out",record.status,<div className="rowactions" key={`${record.id}-actions`}><button className="rowaction" onClick={()=>setEditing({...record})}><Pencil size={14}/>Edit</button><button className="rowaction danger" onClick={()=>remove(record)}><Trash2 size={14}/>Delete</button></div>])}>
   {editing&&<RecordModal title={editing.id?`Edit ${siteLabel(site)} item`:`Add ${siteLabel(site)} item`} onCancel={()=>setEditing(null)} onSave={()=>commit(editing)} onSaveContinue={()=>commit(editing,true)}>
    {access.isAllCompanies?<label>Owner company<select value={editing.companyId} onChange={e=>setEditing({...editing,companyId:e.target.value})}>{companies.filter(c=>siteForCompanyType(c.companyType)===site).map(c=><option key={c.id} value={c.id}>{c.company}</option>)}</select></label>:<label>Owner company<input value={assignedCompany?.company||"Assigned company"} readOnly/><small className="fieldhelp">Automatically assigned from your account.</small></label>}
  ...
  {editingStall&&<RecordModal title={editingStall.id?"Edit food stall":"Add new food stall"} onCancel={()=>setEditingStall(null)} onSave={()=>saveStall(editingStall)}>{access.isAllCompanies&&<label>Owner company<select value={editingStall.companyId} onChange={e=>setEditingStall({...editingStall,companyId:e.target.value})}>{companies.filter(c=>c.companyType==="Food & Beverage").map(c=><option key={c.id} value={c.id}>{c.company}</option>)}</select></label>}<label>Stall name<input value={editingStall.name} onChange={e=>setEditingStall({...editingStall,name:e.target.value})}/></label><label>Business hours<select value={editingStall.open24Hours?"Open 24 hours":"Set opening hours"} onChange={e=>setEditingStall({...editingStall,open24Hours:e.target.value==="Open 24 hours"})}><option>Set opening hours</option><option>Open 24 hours</option></select></label>{!editingStall.open24Hours&&<><label>Open time<input type="time" value={editingStall.openTime} onChange={e=>setEditingStall({...editingStall,openTime:e.target.value})}/></label><label>Close time<input type="time" value={editingStall.closeTime} onChange={e=>setEditingStall({...editingStall,closeTime:e.target.value})}/></label></>}<label>Temporary status<select value={editingStall.temporaryClosed?"Temporarily closed":"Use business hours"} onChange={e=>setEditingStall({...editingStall,temporaryClosed:e.target.value==="Temporarily closed"})}><option>Use business hours</option><option>Temporarily closed</option></select></label>{editingStall.temporaryClosed&&<label className="wide">Closure message<input value={editingStall.closureReason} onChange={e=>setEditingStall({...editingStall,closureReason:e.target.value})} placeholder="Example: Closed for maintenance until Monday"/></label>}<label>Phone<input value={editingStall.phone} onChange={e=>setEditingStall({...editingStall,phone:e.target.value})}/></label><label>WhatsApp<input value={editingStall.whatsapp} onChange={e=>setEditingStall({...editingStall,whatsapp:e.target.value})}/></label><label>Display order<input type="number" min="0" value={editingStall.displayOrder} onChange={e=>setEditingStall({...editingStall,displayOrder:Number(e.target.value)})}/></label><label>Visibility<select value={editingStall.active?"Visible":"Hidden"} onChange={e=>setEditingStall({...editingStall,active:e.target.value==="Visible"})}><option>Visible</option><option>Hidden</option></select></label><label className="wide">Closed days<div className="rowactions">{["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"].map(day=><label key={day} style={{display:"inline-flex",alignItems:"center",gap:6}}><input type="checkbox" checked={editingStall.closedDays.includes(day)} onChange={e=>setEditingStall({...editingStall,closedDays:e.target.checked?[...editingStall.closedDays,day]:editingStall.closedDays.filter(value=>value!==day)})}/>{day.slice(0,3)}</label>)}</div></label><label className="wide">Address<input value={editingStall.address} onChange={e=>setEditingStall({...editingStall,address:e.target.value})}/></label><label className="wide">Google Maps URL<input value={editingStall.mapUrl} onChange={e=>setEditingStall({...editingStall,mapUrl:e.target.value})}/></label><label className="wide">Description<textarea rows={3} value={editingStall.description} onChange={e=>setEditingStall({...editingStall,description:e.target.value})}/></label></RecordModal>}
 </>
}
...
function RecordModal({title,onCancel,onSave,onSaveContinue,children,className=""}:{title:string;onCancel:()=>void;onSave:()=>boolean;onSaveContinue?:()=>boolean;children:ReactNode;className?:string}){const [error,setError]=useState("");useEffect(()=>{sessionStorage.setItem("a3-record-editing","1");const beforeUnload=(event:BeforeUnloadEvent)=>{event.preventDefault();event.returnValue=""};window.addEventListener("beforeunload",beforeUnload);return()=>{sessionStorage.removeItem("a3-record-editing");window.removeEventListener("beforeunload",beforeUnload)}},[]);const run=(action:()=>boolean)=>{if(action())setError("");else setError("Complete the required fields and enter a valid value where applicable.")};return <div className="modalbackdrop" onMouseDown={event=>{if(event.target===event.currentTarget&&window.innerWidth>700&&window.confirm("Discard unsaved changes?"))onCancel()}}><div className={`modal ${className}`} onMouseDown={e=>e.stopPropagation()}><div className="modalhead"><div><span>RECORD MANAGEMENT</span><h2>{title}</h2></div><button className="modalclose" onClick={()=>{if(window.confirm("Discard unsaved changes?"))onCancel()}}><X size={19}/></button></div>{error&&<div className="formerror">{error}</div>}<div className="rulegrid">{children}</div><div className="modalactions"><button className="ghost" onClick={()=>{if(window.confirm("Discard unsaved changes?"))onCancel()}}>Cancel</button>{onSaveContinue&&<button className="ghost savecontinue" onClick={()=>run(onSaveContinue)}><Save size={16}/>Save & continue</button>}<button className="primary" onClick={()=>run(onSave)}><Save size={16}/>Save</button></div></div></div>}
