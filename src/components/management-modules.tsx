...
type CatalogueRecord={id:string;website:string;item:string;category:string;price:number;status:"Published"|"Draft"};
function WebsiteCatalogueRate(){
 const [records,setRecords]=useState<CatalogueRecord[]>(()=>load("a3-catalogue-rates",[])); const [editing,setEditing]=useState<CatalogueRecord|null>(null); useEffect(()=>save("a3-catalogue-rates",records),[records]);
 const openNew=()=>setEditing({id:"",website:"LIMOUSINE.A3GROUP.SG",item:"",category:"Transport",price:0,status:"Published"}); const commit=(record:CatalogueRecord)=>{if(!record.item.trim()||record.price<=0)return false;setRecords(c=>record.id?c.map(r=>r.id===record.id?record:r):[{...record,id:nextId("CAT",c.length)},...c]);setEditing(null);return true};
 return <EditableSection eyebrow="ADMIN · WEBSITE PRICING" title="Website Catalogue Rate" copy="Create, edit and publish catalogue prices across A3 Group websites." action="Add record" onAdd={openNew} headers={["Record","Website","Item","Category","Rate","Status","Actions"]} rows={records.map(r=>[r.id,r.website,r.item,r.category,money2(r.price),r.status,<button className="rowaction" onClick={()=>setEditing({...r})}><Pencil size={14}/>Edit</button>])}>{editing&&<RecordModal title={editing.id?"Edit catalogue rate":"Add catalogue rate"} onCancel={()=>setEditing(null)} onSave={()=>commit(editing)}><label>Website<select value={editing.website} onChange={e=>setEditing({...editing,website:e.target.value})}><option>LIMOUSINE.A3GROUP.SG</option><option>SAKURA.A3GROUP.SG</option><option>FOOD.A3GROUP.SG</option></select></label><label>Catalogue item<input value={editing.item} onChange={e=>setEditing({...editing,item:e.target.value})}/></label><label>Category<select value={editing.category} onChange={e=>setEditing({...editing,category:e.target.value})}><option>Transport</option><option>Restaurant</option><option>Food</option><option>Surcharge</option></select></label><label>Rate (S$)<input type="number" min="0" value={editing.price} onChange={e=>setEditing({...editing,price:Number(e.target.value)})}/></label><label>Status<select value={editing.status} onChange={e=>setEditing({...editing,status:e.target.value as CatalogueRecord["status"]})}><option>Published</option><option>Draft</option></select></label></RecordModal>}</EditableSection>
}
...
function PlatformEarnings(){
 ...
 const [records,setRecords]=useState<PlatformRecord[]>(()=>load("a3-platform-earnings",defaults));const [editing,setEditing]=useState<PlatformRecord|null>(null);const [reportPeriod,setReportPeriod]=useState<ReportPeriod>("THIS_MONTH");useEffect(()=>save("a3-platform-earnings",records),[records]);
 ...
 const commit=(record:PlatformRecord)=>{if(!record.date||!record.platform||record.gross<=0)return false;const saved={...record,net:calculateNet(record)};setRecords(c=>saved.id?c.map(r=>r.id===saved.id?saved:r):[{...saved,id:nextId("PLT",c.length)},...c]);setEditing(null);return true};
...
function loadSharedDrivers(fallback:DriverRecord[]):DriverRecord[]{
 ...
 saveNow("a3-driver-records",result);
 return result;
}
