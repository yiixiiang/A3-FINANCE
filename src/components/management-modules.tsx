...
const LIBERTY_FOOD_SEED_VERSION="2026-07-28-v1";
const LIBERTY_FOOD_STALLS:CatalogueStall[]=[
 {id:"STALL-KL-BAN-MEE",companyId:"",name:"KL TRADITIONAL CHILLI BAN MEE",description:"Traditional KL chilli ban mee, noodles and soups.",imageUrl:"",displayOrder:1,active:true,openTime:"",closeTime:"",closedDays:[],temporaryClosed:false,phone:"",whatsapp:"",address:"",mapUrl:""},
 {id:"STALL-KL-CHICKEN-RICE",companyId:"",name:"KL CHICKEN RICE",description:"Chicken rice, noodles, side dishes and add-ons.",imageUrl:"",displayOrder:2,active:true,openTime:"",closeTime:"",closedDays:[],temporaryClosed:false,phone:"",whatsapp:"",address:"",mapUrl:""},
];
const libertyFoodMenuRows:Array<[string,string,string,string,number,string,boolean]>=[
 ["STALL-KL-BAN-MEE","Noodles","Signature Chilli Ban Mee","辣椒板面",5.90,"Best Seller",true],
 ["STALL-KL-BAN-MEE","Noodles","Prawn Chilli Ban Mee","辣椒虾板面",8.90,"",false],
 ["STALL-KL-BAN-MEE","Noodles","Fishball Noodle","鱼圆面",5.30,"",false],
 ["STALL-KL-BAN-MEE","Noodles","Minced Meat Noodle","肉碎面",5.30,"",false],
 ["STALL-KL-BAN-MEE","Noodles","Laksa","叻沙",5.30,"",false],
 ["STALL-KL-BAN-MEE","Noodles","Lor Mee","卤面",5.90,"",false],
 ["STALL-KL-BAN-MEE","Noodles","Fried Wanton Ban Mee","炸云吞板面",5.90,"",false],
 ["STALL-KL-BAN-MEE","Noodles","Fried Chicken Ban Mee","炸鸡扒板面",8.90,"",false],
 ["STALL-KL-BAN-MEE","Soup","Sliced Fish Soup","鱼片汤",8.90,"",false],
 ["STALL-KL-BAN-MEE","Noodles","Mee Rebus","马来卤面",5.30,"",false],
 ["STALL-KL-BAN-MEE","Noodles","Mee Siam","米暹",5.30,"",false],
 ["STALL-KL-BAN-MEE","Noodles","Hokkien Mee","福建面",6.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Rice","Steamed Chicken Rice","白鸡饭",5.50,"Popular",true],
 ["STALL-KL-CHICKEN-RICE","Rice","Roasted Chicken Rice","烧鸡饭",5.50,"Popular",true],
 ["STALL-KL-CHICKEN-RICE","Rice","Lemon Chicken Rice","柠檬鸡饭",9.00,"",false],
 ["STALL-KL-CHICKEN-RICE","Rice","Chicken Cutlet Rice","鸡排饭",8.00,"",false],
 ["STALL-KL-CHICKEN-RICE","Rice","Fried Chicken Drumstick Rice","炸鸡腿饭",8.00,"",false],
 ["STALL-KL-CHICKEN-RICE","Rice","Salted Egg Chicken Rice","咸蛋鸡饭",8.00,"",false],
 ["STALL-KL-CHICKEN-RICE","Rice","Popcorn Chicken Rice","爆米花鸡饭",8.00,"",false],
 ["STALL-KL-CHICKEN-RICE","Rice","Roasted Pork Rice","烧肉饭",5.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Rice","Char Siew Rice","叉烧饭",5.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Rice","Pork Leg Rice","猪脚饭",8.00,"Best Seller",true],
 ["STALL-KL-CHICKEN-RICE","Rice","Marmite Fish Steak Rice","妈蜜鱼扒饭",7.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Rice","Chicken Thigh Rice","鸡腿饭",6.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Rice","Chicken Tail Rice","鸡尾饭",6.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Noodles","Wanton Noodle","云吞面",5.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Noodles","Roasted Chicken Noodle","烧鸡面",5.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Noodles","Fish Dumpling Noodle","鱼饺面",5.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Soup","Fish Dumpling Soup","鱼饺汤",5.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Other","Chicken Porridge","鸡粥",5.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Side Dishes","Oyster Sauce Veg","油菜",4.00,"",false],
 ["STALL-KL-CHICKEN-RICE","Side Dishes","Bean Sprout","豆芽",4.00,"",false],
 ["STALL-KL-CHICKEN-RICE","Additional Charges","Add Rice (Large)","加饭(大)",1.00,"",false],
 ["STALL-KL-CHICKEN-RICE","Additional Charges","Add Rice (Small)","加饭(小)",0.50,"",false],
 ["STALL-KL-CHICKEN-RICE","Additional Charges","Takeaway","打包",0.50,"",false],
];
function seedLibertyStalls(existing:CatalogueStall[],companyId:string){
 if(!companyId||typeof window==="undefined")return existing;
 const marker=`a3-liberty-food-stalls-${LIBERTY_FOOD_SEED_VERSION}`;
 if(window.localStorage.getItem(marker))return existing;
 const next=[...existing];
 for(const seed of LIBERTY_FOOD_STALLS){
  const found=next.find(row=>row.companyId===companyId&&row.name.trim().toLowerCase()===seed.name.toLowerCase());
  if(!found)next.push({...seed,companyId});
 }
 window.localStorage.setItem(marker,"1");
 return next;
}
function seedLibertyMenu(existing:CatalogueRecord[],companyId:string){
 if(!companyId||typeof window==="undefined")return existing;
 const marker=`a3-liberty-food-menu-${LIBERTY_FOOD_SEED_VERSION}`;
 if(window.localStorage.getItem(marker))return existing;
 const next=[...existing];
 libertyFoodMenuRows.forEach(([stallId,category,item,titleZh,price,description,featured],index)=>{
  const priceKey=catalogueKey(item);
  const found=next.find(row=>row.companyId===companyId&&row.site==="food"&&row.priceKey===priceKey);
  if(found){
   found.stallId=stallId; found.category=category; found.titleZh=found.titleZh||titleZh; found.description=found.description||description; found.displayOrder=found.displayOrder||index+1;
  }else next.push(normaliseCatalogueV4({id:`CAT-LD-${String(index+1).padStart(3,"0")}`,companyId,site:"food",priceKey,item,titleZh,category,stallId,description:featured?description:"",price,currency:"SGD",displayOrder:index+1,available:true,status:"Published"}));
 });
 window.localStorage.setItem(marker,"1");
 return next;
}
...
function WebsiteCatalogueRate(){
 ...
 const libertyCompanyId=companies.find(company=>company.company.trim().toUpperCase()==="LIBERTY DESTINY")?.id||"";
 const [records,setRecords]=useState<CatalogueRecord[]>(()=>seedLibertyMenu(load<Partial<CatalogueRecord>[]>(WEBSITE_CATALOGUE_STORAGE_KEY,[]).map(normaliseCatalogueV4),libertyCompanyId));
 const [stalls,setStalls]=useState<CatalogueStall[]>(()=>seedLibertyStalls(load<Partial<CatalogueStall>[]>(WEBSITE_STALL_STORAGE_KEY,[]).map(row=>({id:String(row.id||""),companyId:String(row.companyId||""),name:String(row.name||""),description:String(row.description||""),imageUrl:String(row.imageUrl||""),displayOrder:Number(row.displayOrder||0),active:row.active!==false,openTime:String(row.openTime||""),closeTime:String(row.closeTime||""),closedDays:Array.isArray(row.closedDays)?row.closedDays:[],temporaryClosed:Boolean(row.temporaryClosed),phone:String(row.phone||""),whatsapp:String(row.whatsapp||""),address:String(row.address||""),mapUrl:String(row.mapUrl||"")})),libertyCompanyId));
 ...
 const categoryOptions=site==="nightclub"?["Tower","Beer","Brandy","Whisky","Vodka","Gin","Rum","Tequila","Wine","Champagne","Cocktails","Promotions","Other"]:site==="limousine"?["Vehicle","Airport Transfer","Point to Point","Hourly Service","Wedding","Package","Other"]:["Rice","Noodles","Soup","Set Meals","Side Dishes","Drinks","Dessert","Additional Charges","Other"];
 ...
 return <>
  ...
  <EditableSection eyebrow={`${siteLabel(site).toUpperCase()} WEBSITE`} title={`${siteLabel(site)} items`} copy={site==="food"?"Menu items and pictures grouped by stall.":site==="nightclub"?"Tower, beer, brandy and any other drink or promotion category.":"Vehicles, transfer services, hourly rates and packages."} action={`Add ${siteLabel(site)} item`} onAdd={openNew} headers={site==="food"?["Photo","Item","Category","Stall","Price","Availability","Status","Actions"]:["Photo","Item","Company","Category","Group","Price","Availability","Status","Actions"]} rows={[...visibleRecords].sort((a,b)=>a.displayOrder-b.displayOrder).map(record=>{const common=[record.imageUrl?<img src={record.imageUrl} alt={record.imageAlt} style={{width:54,height:54,objectFit:"cover",borderRadius:10}}/>:"—",<div key={record.id}><strong>{record.item}</strong><br/><small>{record.titleZh||record.priceKey}</small></div>];const tail=[record.category,site==="food"?(visibleStalls.find(s=>s.id===record.stallId)?.name||"Unassigned"):(record.subgroup||"—"),`${record.currency} ${record.price.toFixed(2)}`,record.available?"Available":"Sold out",record.status,<div className="rowactions" key={`${record.id}-actions`}><button className="rowaction" onClick={()=>setEditing({...record})}><Pencil size={14}/>Edit</button><button className="rowaction danger" onClick={()=>remove(record)}><Trash2 size={14}/>Delete</button></div>];return site==="food"?[...common,...tail]:[...common,companies.find(c=>c.id===record.companyId)?.company||"—",...tail]})}>
   {editing&&<RecordModal title={editing.id?`Edit ${siteLabel(site)} item`:`Add ${siteLabel(site)} item`} onCancel={()=>setEditing(null)} onSave={()=>commit(editing)}>
    {access.isAllCompanies&&site!=="food"&&<label>Owner company<select value={editing.companyId} onChange={e=>setEditing({...editing,companyId:e.target.value})}>{companies.filter(c=>siteForCompanyType(c.companyType)===site).map(c=><option key={c.id} value={c.id}>{c.company}</option>)}</select></label>}
