...
function FleetManagement(){
 ...
 const saveFleet=()=>{const next=normalizedVehicles();setVehicles(next);saveNow(FLEET_STORAGE_KEY,next);setPublishState("saving");setMessage("Fleet and photos saved as draft in Finance CMS.");window.setTimeout(()=>setPublishState(""),900)};
 const publishFleet=async()=>{const next=normalizedVehicles();setVehicles(next);saveNow(FLEET_STORAGE_KEY,next);setPublishState("publishing");setMessage("");try{await uploadStorageKeysToCloud([FLEET_STORAGE_KEY]);const response=await fetch(`/api/public/rate-matrix?t=${Date.now()}`,{cache:"no-store"});const data=await response.json();if(!response.ok||!Array.isArray(data.vehicle_types))throw new Error(data?.error||"Fleet publish verification failed");const publishedNames=new Set(data.vehicle_types.map((item:any)=>String(item.name||"").trim().toLowerCase()));if(next.filter(item=>item.status==="Active").some(item=>!publishedNames.has(item.name.trim().toLowerCase())))throw new Error("One or more active vehicles were not returned by the public API.");setPublishState("success");setMessage("Fleet, photos and capacity published successfully to the limousine website.")}catch(error){setPublishState("error");setMessage(error instanceof Error?error.message:"Unable to publish fleet. Check Cloud & Backup connection.")}};
