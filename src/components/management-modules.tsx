...
type CatalogueRecord={id:string;website:string;priceKey:string;item:string;category:string;price:number;status:"Published"|"Draft"};
const catalogueKey=(value:string)=>value.toLowerCase().trim().replace(/[^a-z0-9]+/g,"-").replace(/^-+|-+$/g,"");
function WebsiteCatalogueRate(){
 const [records,setRecords]=useState<CatalogueRecord[]>(()=>load<CatalogueRecord[]>("a3-catalogue-rates",[]).map(record=>({...record,priceKey:record.priceKey||catalogueKey(record.item)}))); const [editing,setEditing]=useState<CatalogueRecord|null>(null); useEffect(()=>save("a3-catalogue-rates",records),[records]);
 const openNew=()=>setEditing({id:"",website:"FOOD.A3GROUP.SG",priceKey:"",item:"",category:"Food",price:0,status:"Published"});
 const commit=(record:CatalogueRecord)=>{const normalized={...record,priceKey:catalogueKey(record.priceKey||record.item)};if(!normalized.item.trim()||!normalized.priceKey||normalized.price<=0)return false;const duplicate=records.some(row=>row.website===normalized.website&&row.priceKey===normalized.priceKey&&row.id!==normalized.id);if(duplicate){window.alert("This Price Key already exists for the selected website.");return false}setRecords(current=>normalized.id?current.map(row=>row.id===normalized.id?normalized:row):[{...normalized,id:nextId("CAT",current.length)},...current]);setEditing(null);return true};
 const remove=(record:CatalogueRecord)=>{if(!window.confirm(`Delete ${record.item} (${record.priceKey})? The public website will fall back to its built-in price.`))return;setRecords(current=>current.filter(row=>row.id!==record.id))};
 return <EditableSection eyebrow="ADMIN · WEBSITE PRICING" title="Website Catalogue Rate" copy="Publish prices to A3 websites. The Price Key must match the key used by the public website." action="Add record" onAdd={openNew} headers={["Record","Website","Price Key","Item","Category","Rate","Status","Actions"]} rows={records.map(record=>[record.id,record.website,record.priceKey,record.item,record.category,money2(record.price),record.status,<div className="rowactions" key={record.id}><button className="rowaction" onClick={()=>setEditing({...record})}><Pencil size={14}/>Edit</button><button className="rowaction danger" onClick={()=>remove(record)}>Delete</button></div>])}>{editing&&<RecordModal title={editing.id?"Edit catalogue rate":"Add catalogue rate"} onCancel={()=>setEditing(null)} onSave={()=>commit(editing)}><label>Website<select value={editing.website} onChange={e=>setEditing({...editing,website:e.target.value})}><option>LIMOUSINE.A3GROUP.SG</option><option>SAKURA.A3GROUP.SG</option><option>FOOD.A3GROUP.SG</option></select></label><label>Price Key<input value={editing.priceKey} onChange={e=>setEditing({...editing,priceKey:catalogueKey(e.target.value)})} placeholder="chicken-rice-steamed-chicken-rice"/><small>Use lowercase letters, numbers and hyphens only.</small></label><label>Catalogue item<input value={editing.item} onChange={e=>setEditing({...editing,item:e.target.value,priceKey:editing.priceKey||catalogueKey(e.target.value)})}/></label><label>Category<select value={editing.category} onChange={e=>setEditing({...editing,category:e.target.value})}><option>Transport</option><option>Restaurant</option><option>Food</option><option>Surcharge</option></select></label><label>Rate (S$)<input type="number" min="0" step="0.01" value={editing.price} onChange={e=>setEditing({...editing,price:Number(e.target.value)})}/></label><label>Status<select value={editing.status} onChange={e=>setEditing({...editing,status:e.target.value as CatalogueRecord["status"]})}><option>Published</option><option>Draft</option></select></label></RecordModal>}</EditableSection>
}
