...
function TermsManagement(){
 ...
 const publish=()=>{const clean={...draft,title:draft.title.trim()||"Terms & Conditions",version:draft.version.trim()||new Date().toISOString().slice(0,10),lastUpdated:draft.lastUpdated||new Date().toISOString().slice(0,10),publishedAt:new Date().toISOString()};saveNow(TERMS_DRAFT_KEY,clean);saveNow(TERMS_PUBLIC_KEY,clean);setDraft(clean);setNotice("Published. The limousine website will load this version from Finance CMS.")};
 return <><Heading eyebrow="WEBSITE MANAGEMENT" title="Terms & Conditions" copy="Edit, save and publish the terms shown on the limousine website and booking agreement."/>
 <div className="panel"><div className="rulegrid"><label>Page title<input value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})}/></label><label>Version<input value={draft.version} onChange={e=>setDraft({...draft,version:e.target.value})} placeholder="2026-07-29"/></label><label>Last updated<input type="date" value={draft.lastUpdated} onChange={e=>setDraft({...draft,lastUpdated:e.target.value})}/></label><label className="wide">Introductory text<textarea rows={3} value={draft.intro} onChange={e=>setDraft({...draft,intro:e.target.value})}/></label><label className="wide">Booking checkbox wording<textarea rows={3} value={draft.checkboxText} onChange={e=>setDraft({...draft,checkboxText:e.target.value})}/></label><label className="wide">Terms content<textarea rows={28} value={draft.content} onChange={e=>setDraft({...draft,content:e.target.value})}/><small>Use a blank line between sections. The live website keeps the same section order and paragraph breaks.</small></label></div><div className="modalactions"><button className="ghost" type="button" onClick={saveDraft}><Save size={16}/>Save Draft</button><button className="primary" type="button" onClick={publish}><Send size={16}/>Publish to Limousine Website</button></div>{notice&&<p className="formMessage">{notice}</p>}</div></>
}
