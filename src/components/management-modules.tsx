...
import { BadgeDollarSign, CarFront, CheckCircle2, Download, Files, MessageCircle, WalletCards, Users, X, Plus, Pencil, Save, Send, Upload, Eye, Trash2, Printer, ShieldCheck, RefreshCw } from "lucide-react";
...
function RateManagement(){
 const [tab,setTab]=useState<"vehicle"|"fleet"|"charges"|"driver"|"client">("vehicle");
 return <><Heading eyebrow="ADMIN · CENTRAL PRICING" title="Rate Management" copy="Manage website rates, fleet photos, additional charges, job payouts and client contract rates from one unified workspace."/><div className="ratetabs" role="tablist" aria-label="Rate management sections"><button role="tab" aria-selected={tab==="vehicle"} className={tab==="vehicle"?"active":""} onClick={()=>setTab("vehicle")}><CarFront size={17}/>Vehicle Rate</button><button role="tab" aria-selected={tab==="fleet"} className={tab==="fleet"?"active":""} onClick={()=>setTab("fleet")}><Upload size={17}/>Fleet & Photos</button><button role="tab" aria-selected={tab==="charges"} className={tab==="charges"?"active":""} onClick={()=>setTab("charges")}><BadgeDollarSign size={17}/>Additional Charges</button><button role="tab" aria-selected={tab==="driver"} className={tab==="driver"?"active":""} onClick={()=>setTab("driver")}><WalletCards size={17}/>Job Payout Rate</button><button role="tab" aria-selected={tab==="client"} className={tab==="client"?"active":""} onClick={()=>setTab("client")}><Users size={17}/>Client Fix Rate</button></div><div className="rateworkspace">{tab==="vehicle"&&<RateMatrix/>}{tab==="fleet"&&<FleetManagement/>}{tab==="charges"&&<AdditionalCharges/>}{tab==="driver"&&<DriverFixRate/>}{tab==="client"&&<ClientFixRate/>}</div></>
}
...
function inferFixedRateService(item:DocumentLineItem){
 ...
 if(/hour|小时|disposal/.test(text))return "Hourly Disposal (minimum 3 hours)";
 if(/jb|johor|cross border/.test(text))return "Cross Border SG to JB (from)";
...
function ManagedRateMatrix({kind,title,eyebrow,copy,defaults,client}:{kind:string;title:string;eyebrow:string;copy:string;defaults:ManagedRateRule[];client?:string}){
 ...
 const [vehicleOpen,setVehicleOpen]=useState(false); const [newVehicle,setNewVehicle]=useState(""); const [vehicleEditIndex,setVehicleEditIndex]=useState<number|null>(null); const [publishState,setPublishState]=useState<""|"publishing"|"success"|"error">(""); const canManageVehicles=kind==="vehicle";
 ...
 const openAddVehicle=()=>{setVehicleEditIndex(null);setNewVehicle("");setVehicleOpen(true)};
 const openEditVehicle=(index:number)=>{setVehicleEditIndex(index);setNewVehicle(vehicles[index]||"");setVehicleOpen(true)};
 const commitVehicle=()=>{const name=newVehicle.trim();if(!name)return;if(vehicles.some((v,i)=>i!==vehicleEditIndex&&v.toLowerCase()===name.toLowerCase()))return; if(vehicleEditIndex===null){setVehicles(current=>[...current,name]);setRules(current=>current.map(rule=>({...rule,values:[...rule.values,"0"]})));}else{const oldName=vehicles[vehicleEditIndex];setVehicles(current=>current.map((value,index)=>index===vehicleEditIndex?name:value));const fleet=load<FleetVehicle[]>(FLEET_STORAGE_KEY,fleetDefaults).map(item=>item.name.trim().toLowerCase()===oldName.trim().toLowerCase()?{...item,name}:item);saveNow(FLEET_STORAGE_KEY,fleet);} setNewVehicle("");setVehicleEditIndex(null);setVehicleOpen(false)};
 const removeVehicle=(index:number)=>{if(vehicles.length<=1)return;const oldName=vehicles[index];if(!window.confirm(`Delete ${oldName} from Vehicle Rate and Fleet Management?`))return;setVehicles(current=>current.filter((_,i)=>i!==index));setRules(current=>current.map(rule=>({...rule,values:rule.values.filter((_,i)=>i!==index)})));const fleet=load<FleetVehicle[]>(FLEET_STORAGE_KEY,fleetDefaults).filter(item=>item.name.trim().toLowerCase()!==oldName.trim().toLowerCase());saveNow(FLEET_STORAGE_KEY,fleet)};
 const publishVehicleRates=async(useStarter=false)=>{if(kind!=="vehicle")return;const nextRules=useStarter?vehicleRateDefaults.map(rule=>({...rule,values:[...rule.values]})):rules;if(useStarter){setRules(nextRules);setVehicles([...rateVehicleDefaults])}setPublishState("publishing");saveNow(vehicleKey,useStarter?[...rateVehicleDefaults]:vehicles);saveNow(rulesKey,nextRules);try{await new Promise(resolve=>window.setTimeout(resolve,900));const response=await fetch(`/api/public/rate-matrix?t=${Date.now()}`,{cache:"no-store"});const data=await response.json();if(!response.ok||!Array.isArray(data.rate_cards))throw new Error("Publish verification failed");setPublishState("success")}catch{setPublishState("error")}};
 return <><Heading eyebrow={eyebrow} title={title} copy={copy} action="Add rate rule" onAction={openAdd}/>{kind==="vehicle"&&<div className="marketstarter"><div><strong>Singapore market starter rates</strong><span>Load the recommended public rate matrix, then review and edit before publishing.</span></div><button className="primary" onClick={()=>{if(window.confirm("Replace and publish the current website vehicle rates with the market starter rates?"))void publishVehicleRates(true)}}><Send size={15}/>Load & publish rates</button></div>}
 ...
 <div className="rateactions">{canManageVehicles&&<button className="ghost" onClick={openAddVehicle}><Plus size={16}/>Add vehicle type</button>}<span>{canManageVehicles?"Vehicle types, services and prices are editable in SGD.":"Vehicle types and service structure follow Vehicle Rate. Only prices can be changed."}</span><button className="ghost" onClick={()=>{saveNow(vehicleKey,vehicles);saveNow(rulesKey,rules)}}><Save size={16}/>Save draft</button>{canManageVehicles&&<button className="primary" disabled={publishState==="publishing"} onClick={()=>void publishVehicleRates(false)}><Send size={16}/>{publishState==="publishing"?"Publishing…":"Publish to Limousine Website"}</button>}</div>{canManageVehicles&&publishState==="success"&&<div className="publishnotice"><CheckCircle2 size={17}/>Published successfully. The public rate API now returns this matrix.</div>}{canManageVehicles&&publishState==="error"&&<div className="publishnotice error"><X size={17}/>Rates were saved, but public API verification failed. Check Cloud & Backup connection.</div>}
 <div className="panel"><div className="tablewrap"><table className="rate"><thead><tr><th>Rate rule</th><th>Trip type</th>{vehicles.map((vehicle,index)=><th key={`${vehicle}-${index}`}><div className="vehiclehead"><span>{vehicle}</span>{canManageVehicles&&<span className="vehiclehead-actions"><button aria-label={`Edit ${vehicle}`} title="Edit vehicle type" onClick={()=>openEditVehicle(index)}><Pencil size={12}/></button><button aria-label={`Delete ${vehicle}`} title="Delete vehicle type" onClick={()=>removeVehicle(index)}><Trash2 size={12}/></button></span>}</div></th>)}<th>Status</th><th>Actions</th></tr></thead><tbody>{rules.length?rules.map((rule,row)=><tr key={`${rule.service}-${row}`}><td><strong>{rule.service}</strong></td><td><span className="pill">{rule.tripType}</span></td>{vehicles.map((vehicle,col)=><td key={`${vehicle}-${col}`}>{money2(Number(rule.values[col]||0))}</td>)}<td><span className={`status ${rule.status.toLowerCase()}`}>{rule.status}</span></td><td><div className="rowactions"><button className="rowaction" onClick={()=>openEdit(row)}><Pencil size={14}/>Edit</button><button className="rowaction danger" onClick={()=>remove(row)}><Trash2 size={14}/>Delete</button></div></td></tr>):<tr><td colSpan={vehicles.length+4}>No rate rules. Select <strong>Add rate rule</strong>.</td></tr>}</tbody></table></div></div>
 ...
 {vehicleOpen&&canManageVehicles&&<div className="modalbackdrop" onMouseDown={()=>setVehicleOpen(false)}><div className="modal compactmodal" onMouseDown={e=>e.stopPropagation()}><div className="modalhead"><div><span>FLEET CONTROL</span><h2>{vehicleEditIndex===null?"Add vehicle type":"Edit vehicle type"}</h2></div><button className="modalclose" onClick={()=>setVehicleOpen(false)}><X size={19}/></button></div><label>Vehicle type name<input autoFocus value={newVehicle} onChange={e=>setNewVehicle(e.target.value)} placeholder="Example: 9 Seater Premium"/></label><div className="modalactions"><button className="ghost" onClick={()=>{setVehicleOpen(false);setVehicleEditIndex(null)}}>Cancel</button><button className="primary" disabled={!newVehicle.trim()} onClick={commitVehicle}><Save size={16}/>{vehicleEditIndex===null?"Add vehicle":"Save changes"}</button></div></div></div>}</>
}
...
type FleetVehicle={id:string;name:string;passengerCapacity:number;luggageCapacity:number;description:string;imageUrl:string;status:"Active"|"Inactive";displayOrder:number};
type AdditionalCharge={id:string;code:string;name:string;chargeType:"Fixed"|"Percentage"|"Per Hour"|"Per Stop"|"Per Seat"|"Actual Cost";amount:number;description:string;status:"Active"|"Inactive";displayOrder:number};
const FLEET_STORAGE_KEY="a3-limousine-fleet-v1";
...
function FleetManagement(){
 ...
 const commit=()=>{if(!editing?.name.trim())return false;setVehicles(rows=>rows.some(row=>row.id===editing.id)?rows.map(row=>row.id===editing.id?editing:row):[...rows,{...editing,id:`VEH-${Date.now()}`}]);setEditing(null);return true};
 const upload=(file?:File)=>{if(!file||!editing)return;fileToData(file,(_name,imageUrl)=>setEditing(current=>current?{...current,imageUrl}:current))};
 return <><Heading eyebrow="ADMIN · PUBLIC FLEET" title="Fleet & Vehicle Photos" copy="Vehicle photos and specifications published here are displayed automatically on limousine.a3group.sg." action="Add vehicle" onAction={()=>setEditing({id:`VEH-${Date.now()}`,name:"",passengerCapacity:0,luggageCapacity:0,description:"",imageUrl:"",status:"Active",displayOrder:vehicles.length+1})}/><div className="panel"><div className="tablewrap"><table><thead><tr><th>Photo</th><th>Vehicle</th><th>Passengers</th><th>Luggage</th><th>Status</th><th>Order</th><th>Actions</th></tr></thead><tbody>{vehicles.sort((a,b)=>a.displayOrder-b.displayOrder).map(vehicle=><tr key={vehicle.id}><td>{vehicle.imageUrl?<img src={vehicle.imageUrl} alt={vehicle.name} style={{width:92,height:54,objectFit:"cover",borderRadius:8}}/>:"No photo"}</td><td><strong>{vehicle.name}</strong><br/><small>{vehicle.description}</small></td><td>{vehicle.passengerCapacity||"—"}</td><td>{vehicle.luggageCapacity||"—"}</td><td><span className={`status ${vehicle.status.toLowerCase()}`}>{vehicle.status}</span></td><td>{vehicle.displayOrder}</td><td><div className="rowactions"><button className="rowaction" onClick={()=>setEditing({...vehicle})}><Pencil size={14}/>Edit</button><button className="rowaction danger" onClick={()=>setVehicles(rows=>rows.filter(row=>row.id!==vehicle.id))}><Trash2 size={14}/>Delete</button></div></td></tr>)}</tbody></table></div></div>{editing&&<RecordModal title={editing.name?"Edit vehicle":"Add vehicle"} onCancel={()=>setEditing(null)} onSave={commit}><label>Vehicle name<input value={editing.name} onChange={e=>setEditing({...editing,name:e.target.value})}/></label><label>Passenger capacity<input type="number" min="0" value={editing.passengerCapacity} onChange={e=>setEditing({...editing,passengerCapacity:Number(e.target.value)})}/></label><label>Luggage capacity<input type="number" min="0" value={editing.luggageCapacity} onChange={e=>setEditing({...editing,luggageCapacity:Number(e.target.value)})}/></label><label>Display order<input type="number" min="1" value={editing.displayOrder} onChange={e=>setEditing({...editing,displayOrder:Number(e.target.value)})}/></label><label>Status<select value={editing.status} onChange={e=>setEditing({...editing,status:e.target.value as FleetVehicle["status"]})}><option>Active</option><option>Inactive</option></select></label><label>Vehicle photo<input type="file" accept="image/*" onChange={e=>void upload(e.target.files?.[0])}/></label><label className="wide">Description<textarea value={editing.description} onChange={e=>setEditing({...editing,description:e.target.value})}/></label>{editing.imageUrl&&<div className="wide"><img src={editing.imageUrl} alt="Vehicle preview" style={{width:"100%",maxHeight:260,objectFit:"cover",borderRadius:12}}/></div>}</RecordModal>}</>
}
function AdditionalCharges(){
 const [charges,setCharges]=useState<AdditionalCharge[]>(()=>load(CHARGES_STORAGE_KEY,chargeDefaults));
 const [editing,setEditing]=useState<AdditionalCharge|null>(null);
 const [publishState,setPublishState]=useState<""|"publishing"|"success"|"error">("");
 useEffect(()=>save(CHARGES_STORAGE_KEY,charges),[charges]);
 const commit=()=>{if(!editing?.name.trim())return false;setCharges(rows=>rows.some(row=>row.id===editing.id)?rows.map(row=>row.id===editing.id?editing:row):[...rows,{...editing,id:`CHG-${Date.now()}`}]);setEditing(null);return true};
 const publishCharges=async(useStarter=false)=>{const next=useStarter?chargeDefaults.map(item=>({...item})):charges;if(useStarter)setCharges(next);setPublishState("publishing");saveNow(CHARGES_STORAGE_KEY,next);try{await new Promise(resolve=>window.setTimeout(resolve,900));const response=await fetch(`/api/public/rate-matrix?t=${Date.now()}`,{cache:"no-store"});const data=await response.json();if(!response.ok||!Array.isArray(data.additional_charges))throw new Error("Publish verification failed");setPublishState("success")}catch{setPublishState("error")}};
 return <><Heading eyebrow="ADMIN · SURCHARGES" title="Additional Charges" copy="Publish midnight, waiting, extra-stop, child-seat and actual-cost charges alongside the base rate matrix." action="Add charge" onAction={()=>setEditing({id:`CHG-${Date.now()}`,code:"",name:"",chargeType:"Fixed",amount:0,description:"",status:"Active",displayOrder:charges.length+1})}/><div className="marketstarter"><div><strong>Market-aligned additional charges</strong><span>Load a complete Singapore starter list and edit any amount before publishing.</span></div><button className="primary" onClick={()=>{if(window.confirm("Replace and publish the current website additional charges with the market starter list?"))void publishCharges(true)}}><Send size={15}/>Load & publish charges</button></div><div className="rateactions"><span>Changes remain editable until you publish them.</span><button className="ghost" onClick={()=>saveNow(CHARGES_STORAGE_KEY,charges)}><Save size={16}/>Save draft</button><button className="primary" disabled={publishState==="publishing"} onClick={()=>void publishCharges(false)}><Send size={16}/>{publishState==="publishing"?"Publishing…":"Publish to Limousine Website"}</button></div>{publishState==="success"&&<div className="publishnotice"><CheckCircle2 size={17}/>Additional charges published successfully.</div>}{publishState==="error"&&<div className="publishnotice error"><X size={17}/>Charges were saved, but public API verification failed. Check Cloud & Backup connection.</div>}<div className="panel"><div className="tablewrap"><table><thead><tr><th>Code</th><th>Charge</th><th>Method</th><th>Amount</th><th>Status</th><th>Order</th><th>Actions</th></tr></thead><tbody>{charges.sort((a,b)=>a.displayOrder-b.displayOrder).map(charge=><tr key={charge.id}><td>{charge.code}</td><td><strong>{charge.name}</strong><br/><small>{charge.description}</small></td><td>{charge.chargeType}</td><td>{charge.chargeType==="Actual Cost"?"Actual cost":charge.chargeType==="Percentage"?`${charge.amount}%`:money2(charge.amount)}</td><td><span className={`status ${charge.status.toLowerCase()}`}>{charge.status}</span></td><td>{charge.displayOrder}</td><td><div className="rowactions"><button className="rowaction" onClick={()=>setEditing({...charge})}><Pencil size={14}/>Edit</button><button className="rowaction danger" onClick={()=>setCharges(rows=>rows.filter(row=>row.id!==charge.id))}><Trash2 size={14}/>Delete</button></div></td></tr>)}</tbody></table></div></div>{editing&&<RecordModal title={editing.name?"Edit additional charge":"Add additional charge"} onCancel={()=>setEditing(null)} onSave={commit}><label>Code<input value={editing.code} onChange={e=>setEditing({...editing,code:e.target.value.toUpperCase().replace(/\s+/g,"_")})}/></label><label>Charge name<input value={editing.name} onChange={e=>setEditing({...editing,name:e.target.value})}/></label><label>Method<select value={editing.chargeType} onChange={e=>setEditing({...editing,chargeType:e.target.value as AdditionalCharge["chargeType"]})}><option>Fixed</option><option>Percentage</option><option>Per Hour</option><option>Per Stop</option><option>Per Seat</option><option>Actual Cost</option></select></label><label>{editing.chargeType==="Percentage"?"Percentage (%)":"Amount (S$)"}<input type="number" min="0" max={editing.chargeType==="Percentage"?100:undefined} step="0.01" disabled={editing.chargeType==="Actual Cost"} value={editing.amount} onChange={e=>setEditing({...editing,amount:Number(e.target.value)})}/></label><label>Display order<input type="number" min="1" value={editing.displayOrder} onChange={e=>setEditing({...editing,displayOrder:Number(e.target.value)})}/></label><label>Status<select value={editing.status} onChange={e=>setEditing({...editing,status:e.target.value as AdditionalCharge["status"]})}><option>Active</option><option>Inactive</option></select></label><label className="wide">Description<textarea value={editing.description} onChange={e=>setEditing({...editing,description:e.target.value})}/></label></RecordModal>}</>
}
...
type CatalogueStall={id:string;companyId:string;name:string;description:string;imageUrl:string;displayOrder:number;active:boolean;openTime:string;closeTime:string;closedDays:string[];temporaryClosed:boolean;phone:string;whatsapp:string;address:string;mapUrl:string};
type CatalogueReview={id:string;companyId:string;site:CatalogueSite;stallId:string;itemId:string;customerName:string;rating:number;title:string;review:string;reviewDate:string;status:"Pending"|"Approved"|"Rejected"|"Hidden";featured:boolean;verified:boolean;adminReply:string;displayOrder:number};
const WEBSITE_CATALOGUE_STORAGE_KEY="a3-website-catalogue-v4";
const WEBSITE_STALL_STORAGE_KEY="a3-website-food-stalls-v4";
const WEBSITE_REVIEW_STORAGE_KEY="a3-website-reviews-v5";
...
function WebsiteCatalogueRate(){
 ...
 const [stalls,setStalls]=useState<CatalogueStall[]>(()=>load<Partial<CatalogueStall>[]>(WEBSITE_STALL_STORAGE_KEY,[]).map(row=>({id:String(row.id||""),companyId:String(row.companyId||""),name:String(row.name||""),description:String(row.description||""),imageUrl:String(row.imageUrl||""),displayOrder:Number(row.displayOrder||0),active:row.active!==false,openTime:String(row.openTime||""),closeTime:String(row.closeTime||""),closedDays:Array.isArray(row.closedDays)?row.closedDays:[],temporaryClosed:Boolean(row.temporaryClosed),phone:String(row.phone||""),whatsapp:String(row.whatsapp||""),address:String(row.address||""),mapUrl:String(row.mapUrl||"")})));
 const [reviews,setReviews]=useState<CatalogueReview[]>(()=>load<CatalogueReview[]>(WEBSITE_REVIEW_STORAGE_KEY,[]));
 ...
 const [editingReview,setEditingReview]=useState<CatalogueReview|null>(null);
 useEffect(()=>save(WEBSITE_CATALOGUE_STORAGE_KEY,records),[records]);
 useEffect(()=>save(WEBSITE_STALL_STORAGE_KEY,stalls),[stalls]);
 useEffect(()=>save(WEBSITE_REVIEW_STORAGE_KEY,reviews),[reviews]);
 ...
 const visibleReviews=reviews.filter(review=>review.site===site&&(access.isAllCompanies||review.companyId===access.companyId));
 ...
 const openReview=()=>setEditingReview({id:"",companyId:ownerCompanyId,site,stallId:site==="food"?(visibleStalls[0]?.id||""):"",itemId:"",customerName:"",rating:5,title:"",review:"",reviewDate:new Date().toISOString().slice(0,10),status:"Pending",featured:false,verified:false,adminReply:"",displayOrder:visibleReviews.length+1});
 const saveReview=(review:CatalogueReview)=>{if(!review.companyId||!review.customerName.trim()||!review.review.trim()||review.rating<1||review.rating>5)return false;setReviews(current=>review.id?current.map(row=>row.id===review.id?review:row):[{...review,id:nextId("REV",current.length)},...current]);setEditingReview(null);return true};
 const removeReview=(review:CatalogueReview)=>{if(window.confirm(`Delete review by ${review.customerName}?`))setReviews(current=>current.filter(row=>row.id!==review.id))};
 ...
 return <>
  ...
  <div className="documenttoolbar"><strong>{siteLabel(site)} Catalogue</strong><span>{visibleRecords.length} item(s)</span>{site==="food"&&<button className="ghost" onClick={()=>setEditingStall({id:"",companyId:ownerCompanyId,name:"",description:"",imageUrl:"",displayOrder:visibleStalls.length+1,active:true,openTime:"",closeTime:"",closedDays:[],temporaryClosed:false,phone:"",whatsapp:"",address:"",mapUrl:""})}><Plus size={16}/>Add new stall</button>}<button className="rowaction danger" onClick={clearCatalogue}><Trash2 size={15}/>Remove all menu & pictures</button></div>
  ...
  <EditableSection eyebrow={`${siteLabel(site).toUpperCase()} WEBSITE`} title="Reviews" copy="Approve, feature, reply to, hide or delete customer reviews before they appear publicly." action="Add review" onAdd={openReview} headers={["Customer","Rating","Related to","Date","Status","Featured","Actions"]} rows={[...visibleReviews].sort((a,b)=>a.displayOrder-b.displayOrder).map(review=>[<div key={review.id}><strong>{review.customerName}</strong><br/><small>{review.title||review.review.slice(0,55)}</small></div>,`${"★".repeat(review.rating)}${"☆".repeat(5-review.rating)}`,site==="food"?(visibleStalls.find(stall=>stall.id===review.stallId)?.name||"General"):(review.itemId||"General"),review.reviewDate,review.status,review.featured?"Yes":"No",<div className="rowactions" key={`${review.id}-actions`}><button className="rowaction" onClick={()=>setEditingReview({...review})}><Pencil size={14}/>Edit</button><button className="rowaction danger" onClick={()=>removeReview(review)}><Trash2 size={14}/>Delete</button></div>])}>{editingReview&&<RecordModal title={editingReview.id?"Edit review":"Add review"} onCancel={()=>setEditingReview(null)} onSave={()=>saveReview(editingReview)}>{access.isAllCompanies&&<label>Owner company<select value={editingReview.companyId} onChange={e=>setEditingReview({...editingReview,companyId:e.target.value})}>{companies.filter(c=>siteForCompanyType(c.companyType)===site).map(c=><option key={c.id} value={c.id}>{c.company}</option>)}</select></label>}{site==="food"&&<label>Stall<select value={editingReview.stallId} onChange={e=>setEditingReview({...editingReview,stallId:e.target.value})}><option value="">General</option>{visibleStalls.map(stall=><option key={stall.id} value={stall.id}>{stall.name}</option>)}</select></label>}<label>Customer name<input value={editingReview.customerName} onChange={e=>setEditingReview({...editingReview,customerName:e.target.value})}/></label><label>Rating<select value={editingReview.rating} onChange={e=>setEditingReview({...editingReview,rating:Number(e.target.value)})}>{[5,4,3,2,1].map(value=><option key={value} value={value}>{value} star{value===1?"":"s"}</option>)}</select></label><label>Review date<input type="date" value={editingReview.reviewDate} onChange={e=>setEditingReview({...editingReview,reviewDate:e.target.value})}/></label><label>Status<select value={editingReview.status} onChange={e=>setEditingReview({...editingReview,status:e.target.value as CatalogueReview["status"]})}><option>Pending</option><option>Approved</option><option>Rejected</option><option>Hidden</option></select></label><label>Featured<select value={editingReview.featured?"Yes":"No"} onChange={e=>setEditingReview({...editingReview,featured:e.target.value==="Yes"})}><option>Yes</option><option>No</option></select></label><label>Verified purchase<select value={editingReview.verified?"Yes":"No"} onChange={e=>setEditingReview({...editingReview,verified:e.target.value==="Yes"})}><option>Yes</option><option>No</option></select></label><label>Display order<input type="number" min="0" value={editingReview.displayOrder} onChange={e=>setEditingReview({...editingReview,displayOrder:Number(e.target.value)})}/></label><label className="wide">Review title<input value={editingReview.title} onChange={e=>setEditingReview({...editingReview,title:e.target.value})}/></label><label className="wide">Review<textarea rows={4} value={editingReview.review} onChange={e=>setEditingReview({...editingReview,review:e.target.value})}/></label><label className="wide">Admin reply<textarea rows={3} value={editingReview.adminReply} onChange={e=>setEditingReview({...editingReview,adminReply:e.target.value})}/></label></RecordModal>}</EditableSection>
  {editingStall&&<RecordModal title={editingStall.id?"Edit food stall":"Add new food stall"} onCancel={()=>setEditingStall(null)} onSave={()=>saveStall(editingStall)}>{access.isAllCompanies&&<label>Owner company<select value={editingStall.companyId} onChange={e=>setEditingStall({...editingStall,companyId:e.target.value})}>{companies.filter(c=>c.companyType==="Food & Beverage").map(c=><option key={c.id} value={c.id}>{c.company}</option>)}</select></label>}<label>Stall name<input value={editingStall.name} onChange={e=>setEditingStall({...editingStall,name:e.target.value})}/></label><label>Open time<input type="time" value={editingStall.openTime} onChange={e=>setEditingStall({...editingStall,openTime:e.target.value})}/></label><label>Close time<input type="time" value={editingStall.closeTime} onChange={e=>setEditingStall({...editingStall,closeTime:e.target.value})}/></label><label>Temporary status<select value={editingStall.temporaryClosed?"Temporarily closed":"Use business hours"} onChange={e=>setEditingStall({...editingStall,temporaryClosed:e.target.value==="Temporarily closed"})}><option>Use business hours</option><option>Temporarily closed</option></select></label><label>Phone<input value={editingStall.phone} onChange={e=>setEditingStall({...editingStall,phone:e.target.value})}/></label><label>WhatsApp<input value={editingStall.whatsapp} onChange={e=>setEditingStall({...editingStall,whatsapp:e.target.value})}/></label><label>Display order<input type="number" min="0" value={editingStall.displayOrder} onChange={e=>setEditingStall({...editingStall,displayOrder:Number(e.target.value)})}/></label><label>Visibility<select value={editingStall.active?"Visible":"Hidden"} onChange={e=>setEditingStall({...editingStall,active:e.target.value==="Visible"})}><option>Visible</option><option>Hidden</option></select></label><label className="wide">Closed days<div className="rowactions">{["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"].map(day=><label key={day} style={{display:"inline-flex",alignItems:"center",gap:6}}><input type="checkbox" checked={editingStall.closedDays.includes(day)} onChange={e=>setEditingStall({...editingStall,closedDays:e.target.checked?[...editingStall.closedDays,day]:editingStall.closedDays.filter(value=>value!==day)})}/>{day.slice(0,3)}</label>)}</div></label><label className="wide">Address<input value={editingStall.address} onChange={e=>setEditingStall({...editingStall,address:e.target.value})}/></label><label className="wide">Google Maps URL<input value={editingStall.mapUrl} onChange={e=>setEditingStall({...editingStall,mapUrl:e.target.value})}/></label><label className="wide">Description<textarea rows={3} value={editingStall.description} onChange={e=>setEditingStall({...editingStall,description:e.target.value})}/></label></RecordModal>}
 </>
}
...
type DriverApplication={id:string;name:string;mobile:string;email?:string;nric?:string;dob?:string;address?:string;carModel:string;vehicleType:string;carPlate:string;phvCompany?:string;bankName?:string;bankAccount?:string;paynow?:string;documents?:Record<string,{name:string;type:string;data:string}>;status:"Pending Review"|"Need More Information"|"Approved"|"Rejected";submittedAt:string;updatedAt:string;driverId?:string;username?:string;temporaryPassword?:string};
const DRIVER_APPLICATION_STORAGE_KEY="a3-driver-applications-v1";
...
function DriverManagement(){
 ...
 const [applications,setApplications]=useState<DriverApplication[]>(()=>load(DRIVER_APPLICATION_STORAGE_KEY,[]));
 ...
 useEffect(()=>save(DRIVER_APPLICATION_STORAGE_KEY,applications),[applications]);
 useEffect(()=>{const refresh=()=>setApplications(load(DRIVER_APPLICATION_STORAGE_KEY,[]));window.addEventListener("storage",refresh);window.addEventListener("a3-cloud-synced",refresh as EventListener);return()=>{window.removeEventListener("storage",refresh);window.removeEventListener("a3-cloud-synced",refresh as EventListener)}},[]);
 const approveApplication=(application:DriverApplication)=>{if(application.status==="Approved")return;const driverId=nextId("DRV",drivers.length);const username=(application.email?.split("@")[0]||application.mobile.replace(/\D/g,"")||driverId).toLowerCase();const temporaryPassword=`A3${Math.random().toString(36).slice(2,8)}!`;const driver:DriverRecord={id:driverId,name:application.name,mobile:application.mobile,carModel:application.carModel||models.find(m=>m.active)?.model||"",vehicleType:application.vehicleType||modelType(application.carModel),carPlate:application.carPlate,licenceName:application.documents?.licence?.name||"",licenceData:application.documents?.licence?.data||"",pdvlStatus:"Active",pdvlExpiry:"",bankName:application.bankName||"",bankAccount:application.bankAccount||"",paynow:application.paynow||application.mobile,commission:0,shopRebate:false,joinDate:new Date().toISOString().slice(0,10),status:"Active"};setDrivers(list=>[...list,driver]);const users=normalizeUserRecords(load(USER_ACCESS_STORAGE_KEY,[DEFAULT_ADMIN_USER]));const uniqueUsername=users.some(u=>u.username===username)?`${username}.${driverId.toLowerCase()}`:username;const account=normalizeUserRecord({id:nextId("USR",users.length),username:uniqueUsername,name:application.name,email:application.email||"",password:temporaryPassword,role:"DRIVER",accessScope:"OWN_RECORDS",companyId:"",driverId,visibleModules:defaultModuleIdsForRole("DRIVER"),status:"Active"});saveNow(USER_ACCESS_STORAGE_KEY,[...users,account]);window.dispatchEvent(new Event(USER_ACCESS_UPDATED_EVENT));setApplications(list=>list.map(item=>item.id===application.id?{...item,status:"Approved",driverId,username:uniqueUsername,temporaryPassword,updatedAt:new Date().toISOString()}:item));alert(`Driver approved and account created.\nUsername: ${uniqueUsername}\nTemporary password: ${temporaryPassword}`)};
 const updateApplicationStatus=(id:string,status:DriverApplication["status"])=>setApplications(list=>list.map(item=>item.id===id?{...item,status,updatedAt:new Date().toISOString()}:item));
 ...
 return <><Heading eyebrow="ADMIN · DRIVER & VEHICLE MAPPING" title="Driver Management" copy="Review public driver applications, automatically create approved driver accounts, and manage the active driver directory." action="Add Driver" onAction={()=>setEditing({id:"",name:"",mobile:"",carModel:models.find(m=>m.active)?.model||"",vehicleType:models.find(m=>m.active)?.vehicleType||"",carPlate:"",licenceName:"",licenceData:"",pdvlStatus:"Active",pdvlExpiry:"",bankName:"",bankAccount:"",paynow:"",commission:0,shopRebate:false,joinDate:new Date().toISOString().slice(0,10),status:"Active"})}/><div className="panel"><div className="panelhead"><div><span>PUBLIC SIGN-UP</span><h2>Driver applications</h2><small>Share: https://finance.a3group.sg/driver-signup</small></div></div><div className="tablewrap"><table><thead><tr><th>Applicant</th><th>Vehicle</th><th>Documents</th><th>Status</th><th>Account</th><th>Actions</th></tr></thead><tbody>{applications.length?applications.map(application=><tr key={application.id}><td><strong>{application.name}</strong><br/><small>{application.mobile} · {application.email||"No email"}<br/>{application.id}</small></td><td>{application.carModel||"—"}<br/><small>{application.vehicleType||"—"} · {application.carPlate}</small></td><td>{Object.keys(application.documents||{}).length} uploaded<br/><small>Licence · PDVL · Vehicle front · PHV decal required</small></td><td><span className={`status ${application.status.toLowerCase().replaceAll(" ","")}`}>{application.status}</span></td><td>{application.driverId?<><strong>{application.driverId}</strong><br/><small>{application.username}<br/>Temporary password: {application.temporaryPassword}</small></>:"Not created"}</td><td><div className="rowactions">{application.status!=="Approved"&&<button className="rowaction" onClick={()=>approveApplication(application)}><CheckCircle2 size={14}/>Approve & Create Account</button>}{application.status==="Pending Review"&&<button className="rowaction" onClick={()=>updateApplicationStatus(application.id,"Need More Information")}>Need Info</button>}{application.status!=="Approved"&&<button className="rowaction danger" onClick={()=>updateApplicationStatus(application.id,"Rejected")}>Reject</button>}</div></td></tr>):<tr><td colSpan={6}>No applications yet. Share the public sign-up link with drivers.</td></tr>}</tbody></table></div></div><div className="panel"><div className="panelhead"><div><span>Driver records</span><h2>Active driver directory</h2></div></div><div className="tablewrap"><table><thead><tr>{["Driver","Mobile","Car Model","Vehicle Type","Car Plate","PDVL","Commission","Shop Rebate","Status","Action"].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{drivers.map(d=><tr key={d.id}><td><strong>{d.name}</strong><br/><small>{d.id}</small></td><td>{d.mobile}</td><td>{d.carModel}</td><td>{d.vehicleType}</td><td>{d.carPlate}</td><td>{d.pdvlStatus}<br/><small>{d.pdvlExpiry||"No expiry"}</small></td><td>{d.commission}%</td><td>{d.shopRebate?"On":"Off"}</td><td><span className={`status ${d.status.toLowerCase()}`}>{d.status}</span></td><td><button className="rowaction" onClick={()=>setEditing({...d})}><Pencil size={14}/>Edit</button> <button className="rowaction danger" onClick={()=>setDrivers(list=>list.filter(x=>x.id!==d.id))}><Trash2 size={14}/>Delete</button></td></tr>)}</tbody></table></div></div><div className="panel"><div className="panelhead"><div><span>Vehicle Type Setup</span><h2>Car model mapping library</h2></div><button className="primary" onClick={()=>setMapping({id:"",model:"",vehicleType:vehicleTypes[0],active:true})}><Plus size={16}/>Add Model</button></div><div className="tablewrap"><table><thead><tr><th>Car Model</th><th>Automatic Vehicle Type</th><th>Status</th><th>Action</th></tr></thead><tbody>{models.map(m=><tr key={m.id}><td>{m.model}</td><td>{m.vehicleType}</td><td>{m.active?"Active":"Inactive"}</td><td><button className="rowaction" onClick={()=>setMapping({...m})}><Pencil size={14}/>Edit</button> <button className="rowaction danger" onClick={()=>setModels(list=>list.filter(x=>x.id!==m.id))}><Trash2 size={14}/>Delete</button></td></tr>)}</tbody></table></div></div>{editing&&<RecordModal title={editing.id?"Edit Driver":"Add Driver"} onCancel={()=>setEditing(null)} onSave={()=>saveDriver(editing)}><label>Driver Name<input value={editing.name} onChange={e=>setEditing({...editing,name:e.target.value})}/></label><label>Mobile<input value={editing.mobile} onChange={e=>setEditing({...editing,mobile:e.target.value})}/></label><label>Car Model<select value={editing.carModel} onChange={e=>setEditing({...editing,carModel:e.target.value,vehicleType:modelType(e.target.value)})}>{models.filter(m=>m.active).map(m=><option key={m.id}>{m.model}</option>)}</select></label><label>Vehicle Type<input value={editing.vehicleType} readOnly/></label><label>Car Plate<input value={editing.carPlate} onChange={e=>setEditing({...editing,carPlate:e.target.value.toUpperCase()})}/></label><label>Driving Licence<input type="file" accept="image/*,.pdf" onChange={e=>{const f=e.target.files?.[0];if(f)fileToData(f,(name,data)=>setEditing({...editing,licenceName:name,licenceData:data}))}}/>{editing.licenceName&&<small>{editing.licenceName}</small>}</label><label>PDVL Status<select value={editing.pdvlStatus} onChange={e=>setEditing({...editing,pdvlStatus:e.target.value as DriverRecord["pdvlStatus"]})}><option>Active</option><option>Inactive</option></select></label><label>PDVL Expiry<input type="date" value={editing.pdvlExpiry} onChange={e=>setEditing({...editing,pdvlExpiry:e.target.value})}/></label><label>Bank Name<input value={editing.bankName} onChange={e=>setEditing({...editing,bankName:e.target.value})}/></label><label>Bank Account<input value={editing.bankAccount} onChange={e=>setEditing({...editing,bankAccount:e.target.value})}/></label><label>PayNow<input value={editing.paynow} onChange={e=>setEditing({...editing,paynow:e.target.value})}/></label><label>Commission / Rebate %<input type="number" min="0" max="100" value={editing.commission} onChange={e=>setEditing({...editing,commission:Number(e.target.value)})}/></label><label>Shop Rebate<select value={editing.shopRebate?"On":"Off"} onChange={e=>setEditing({...editing,shopRebate:e.target.value==="On"})}><option>On</option><option>Off</option></select></label><label>Join Date<input type="date" value={editing.joinDate} onChange={e=>setEditing({...editing,joinDate:e.target.value})}/></label><label>Status<select value={editing.status} onChange={e=>setEditing({...editing,status:e.target.value as DriverRecord["status"]})}><option>Active</option><option>Inactive</option><option>Suspended</option></select></label></RecordModal>}{mapping&&<RecordModal title={mapping.id?"Edit Car Model Mapping":"Add Car Model Mapping"} onCancel={()=>setMapping(null)} onSave={()=>saveMap(mapping)}><label>Car Model<input value={mapping.model} onChange={e=>setMapping({...mapping,model:e.target.value})}/></label><label>Vehicle Type<select value={mapping.vehicleType} onChange={e=>setMapping({...mapping,vehicleType:e.target.value})}>{vehicleTypes.map(v=><option key={v}>{v}</option>)}</select></label><label>Status<select value={mapping.active?"Active":"Inactive"} onChange={e=>setMapping({...mapping,active:e.target.value==="Active"})}><option>Active</option><option>Inactive</option></select></label></RecordModal>}</>
}


type WebsiteTermsRecord={title:string;intro:string;content:string;version:string;lastUpdated:string;checkboxText:string;publishedAt?:string};
const TERMS_DRAFT_KEY="a3-limousine-terms-draft-v1";
const TERMS_PUBLIC_KEY="a3-limousine-terms-public-v1";
const DEFAULT_WEBSITE_TERMS:WebsiteTermsRecord={
 title:"Terms & Conditions",
 intro:"These terms apply to limousine, airport transfer, hourly disposal, point-to-point and Singapore–Johor Bahru bookings made with A3 Group SG.",
 version:"2026-07-29",
 lastUpdated:"2026-07-29",
 checkboxText:"I agree to the Terms & Conditions and acknowledge the booking, cancellation, waiting-time, luggage and cross-border policies.",
 content:`Important booking notice
All online bookings are requests and remain pending until availability and the final amount are confirmed by our team.

1. Booking lead time
Online bookings must be submitted more than 6 hours before the scheduled pickup time. A booking made exactly 6 hours before pickup is not accepted. Urgent bookings should be made by telephone or WhatsApp and remain subject to availability.

2. Rates and payment
Displayed prices are starting or base prices unless expressly stated otherwise. ERP, parking, tolls, waiting time, additional stops, remote-area fees and other published surcharges may be added. A booking is confirmed only after our team issues confirmation and any required payment is received.

3. Passenger and luggage capacity
The customer must select a vehicle suitable for the declared passenger and luggage quantities. Unsafe overloading is not permitted.

4. Airport transfers and waiting time
Airport arrivals include up to 60 minutes complimentary waiting from confirmed landing time unless otherwise stated. Other transfers include up to 15 minutes. Additional waiting is chargeable.

5. Hourly disposal
Hourly disposal has a minimum booking of three hours. Extra time is billed at the published hourly rate.

6. Cross-border travel
Singapore to Johor Bahru prices are from the published starting price and may vary by destination, route, timing, immigration conditions, stops and tolls.

7. Cancellation, refund and no-show
Cancellation and refund eligibility depends on the notice period and costs already incurred. Late cancellation and no-show bookings may be charged up to the full confirmed amount.

8. Conduct, cleaning and damage
Smoking, illegal activity and unsafe conduct are prohibited. Cleaning or repair costs caused by passengers may be charged.

9. Delays and vehicle substitution
Traffic, weather, accidents, immigration queues and events outside our control may cause delays. We may substitute an equivalent or higher-capacity vehicle when necessary.

10. Privacy and booking records
Customer details are used to arrange and fulfil the service. Each booking records the accepted terms version and acceptance time.`,
};
function TermsManagement(){
 const [draft,setDraft]=useState<WebsiteTermsRecord>(()=>load(TERMS_DRAFT_KEY,load(TERMS_PUBLIC_KEY,DEFAULT_WEBSITE_TERMS)));
 const [notice,setNotice]=useState("");
 const [activeTab,setActiveTab]=useState<"content"|"settings">("content");
 const sections=useMemo(()=>String(draft.content||"").split(/\n\s*\n/).map(block=>block.trim()).filter(Boolean).map((block,index)=>{const lines=block.split("\n").map(line=>line.trim()).filter(Boolean);const first=lines[0]||`Section ${index+1}`;const heading=lines.length>1||/^(Important|\d+\.|[A-Z][^.!?]{2,70})$/.test(first)?first:`Section ${index+1}`;const body=heading===first?lines.slice(1).join("\n"):lines.join("\n");return {heading,body}}),[draft.content]);
 const saveDraft=()=>{saveNow(TERMS_DRAFT_KEY,draft);setNotice("Draft saved. The live website has not changed.")};
 const publish=()=>{const clean={...draft,title:draft.title.trim()||"Terms & Conditions",version:draft.version.trim()||new Date().toISOString().slice(0,10),lastUpdated:draft.lastUpdated||new Date().toISOString().slice(0,10),publishedAt:new Date().toISOString()};saveNow(TERMS_DRAFT_KEY,clean);saveNow(TERMS_PUBLIC_KEY,clean);setDraft(clean);setNotice("Published successfully. The limousine website will load this version from Finance CMS.")};
 return <><Heading eyebrow="WEBSITE MANAGEMENT" title="Terms & Conditions" copy="Create, review and publish a professional service-policy page for the limousine website."/>
 <div className="termsAdminShell">
  <div className="termsAdminTopbar"><div><span className="termsAdminStatus"><i/>Website policy editor</span><h2>{draft.title||"Untitled policy"}</h2><p>Keep customer-facing terms clear, structured and easy to review before booking.</p></div><div className="termsAdminActions"><button className="ghost" type="button" onClick={saveDraft}><Save size={16}/>Save Draft</button><button className="primary" type="button" onClick={publish}><Send size={16}/>Publish Live</button></div></div>
  {notice&&<div className="termsAdminNotice">{notice}</div>}
  <div className="termsAdminGrid">
   <section className="termsEditorCard">
    <div className="termsEditorTabs"><button className={activeTab==="content"?"active":""} onClick={()=>setActiveTab("content")}>Policy content</button><button className={activeTab==="settings"?"active":""} onClick={()=>setActiveTab("settings")}>Page settings</button></div>
    {activeTab==="content"?<div className="termsEditorBody"><label>Introduction<textarea rows={4} value={draft.intro} onChange={e=>setDraft({...draft,intro:e.target.value})} placeholder="A short summary shown below the page title."/></label><label>Terms sections<textarea className="termsMainTextarea" rows={30} value={draft.content} onChange={e=>setDraft({...draft,content:e.target.value})}/><small>Format each section as a heading on the first line, followed by its paragraph. Leave one blank line between sections.</small></label></div>:<div className="termsEditorBody termsSettingsGrid"><label>Page title<input value={draft.title} onChange={e=>setDraft({...draft,title:e.target.value})}/></label><label>Policy version<input value={draft.version} onChange={e=>setDraft({...draft,version:e.target.value})} placeholder="2026-07-29"/></label><label>Last updated<input type="date" value={draft.lastUpdated} onChange={e=>setDraft({...draft,lastUpdated:e.target.value})}/></label><label className="wide">Booking checkbox wording<textarea rows={4} value={draft.checkboxText} onChange={e=>setDraft({...draft,checkboxText:e.target.value})}/><small>This sentence appears beside the mandatory agreement checkbox on the booking page.</small></label></div>}
   </section>
   <aside className="termsPreviewCard"><div className="termsPreviewHead"><div><span>LIVE PREVIEW</span><strong>Limousine website</strong></div><span className="termsPreviewBadge">Draft</span></div><div className="termsPreviewCanvas"><p className="termsPreviewEyebrow">SERVICE POLICY</p><h2>{draft.title||"Terms & Conditions"}</h2><p className="termsPreviewIntro">{draft.intro||"Add an introduction for customers."}</p><div className="termsPreviewMeta"><span>Version {draft.version||"—"}</span><span>Updated {draft.lastUpdated||"—"}</span></div><nav className="termsPreviewToc"><strong>On this page</strong>{sections.slice(0,6).map((section,index)=><span key={index}>{section.heading}</span>)}</nav><div className="termsPreviewSections">{sections.slice(0,4).map((section,index)=><article key={index}><b>{String(index+1).padStart(2,"0")}</b><div><h3>{section.heading.replace(/^\d+\.\s*/,"")}</h3><p>{section.body||"Add the section details here."}</p></div></article>)}</div>{sections.length>4&&<small className="termsPreviewMore">+ {sections.length-4} more sections</small>}</div></aside>
  </div>
 </div></>
}

const names:Record<string,[string,string,string]>={reports:["REPORTING","Profit & Loss","Live income, expense, invoice and quotation tallies with net profit or loss."],balancesheet:["REPORTING","Balance Sheet","Automatic partner opening, current-period and closing balances."],invoice:["DOCUMENTS","Invoice · English / 中文 A4 PDF","Create bilingual A4 invoices and print or save them as PDF."],quotation:["DOCUMENTS","Quotation · English / 中文 A4 PDF","Create bilingual A4 quotations using your company identity and approved rates."],payout:["DRIVER PRIVATE","Driver Report Payout","Review trip earnings, deductions, incentives and payout status."],rebate:["DRIVER PRIVATE","Driver 10% Rebate","Track qualifying revenue and automatic 10% rebate calculations."],network:["DRIVER PRIVATE","Driver Network","View active drivers, coverage, availability and fleet capability."],clientsetup:["ADMIN","Client Management","Maintain client identity and billing contact details."],driverfix:["ADMIN","Job Payout Rate","Maintain fixed driver pay by job passenger tier."],clientfix:["ADMIN","Client Fix Rate","Maintain client-specific contracted prices."],drivers:["ADMIN","Driver Management","Manage onboarding, documents, vehicles and account status."],catalogue:["ADMIN","Website Catalogue","Manage separate Food, Nightclub and Limousine website catalogues."],terms:["WEBSITE MANAGEMENT","Terms & Conditions","Edit and publish limousine website terms."],company:["ADMIN","Company Management","Configure company identity, GST, currency and document preferences."],access:["ADMIN","User Access","Manage role-based access for administrators, operations staff and drivers."]};
