...
import { uploadStorageKeysToCloud } from "@/lib/supabase-cloud";
...
function ManagedRateMatrix({kind,title,eyebrow,copy,defaults,client}:{kind:string;title:string;eyebrow:string;copy:string;defaults:ManagedRateRule[];client?:string}){
 ...
 return <><Heading eyebrow={eyebrow} title={title} copy={copy} action="Add rate rule" onAction={openAdd}/>{kind==="vehicle"&&<div className="marketstarter"><div><strong>Singapore market starter rates</strong><span>Load the recommended public rate matrix, then review and edit before publishing.</span></div><button className="ghost" onClick={()=>{if(window.confirm("Replace the current draft with the market starter rates?")){const nextVehicles=[...rateVehicleDefaults];const nextRules=vehicleRateDefaults.map(rule=>({...rule,values:[...rule.values]}));setVehicles(nextVehicles);setRules(nextRules);saveNow(vehicleKey,nextVehicles);saveNow(rulesKey,nextRules)}}}><Download size={15}/>Load starter rates</button></div>}
