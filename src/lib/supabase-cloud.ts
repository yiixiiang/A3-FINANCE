...
export async function synchronizeCloudNow(){
 ...
 const cloudRecords:Array<{storage_key:string;value:unknown}>=p.records||[];
 const cloudKeys=new Set(cloudRecords.map(record=>record.storage_key));
 applyCloudRecords(cloudRecords);
 const localOnly=localRecords().filter(record=>!cloudKeys.has(record.storage_key));
 if(localOnly.length)await api("push",{records:localOnly});
 stamp();
 emit("connected");
...
export async function restoreCloudBackup(backupId:string){const p=await api("restore-backup",{backupId});applyCloudRecords(Object.entries(p.payload||{}).map(([storage_key,value])=>({storage_key,value})));await uploadAllLocalDataToCloud();return verifyCloudConnection()}
